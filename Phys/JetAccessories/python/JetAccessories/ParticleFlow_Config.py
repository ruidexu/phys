...
from Configurables import (
    GaudiSequencer, TrackSelector, DelegatingTrackSelector, ParticleFlow,
    ...
    PhotonMatchAlg, PVRelatorAlg, ChargedProtoParticleMaker, FilterDesktop)


class ParticleFlowConf(object):
    def __init__(self,
                 _name,
                 _InputParticles=[
        ...
        self.MCCor = _MCCor
        self.PFSeq = GaudiSequencer(
            _name + "Particles", IgnoreFilterPassed=True)
        self.algorithms = []
    ...
    def setupPF(self):
        ...
        if "Velo" in list(self.paramDef['TrackSelector'].
                          keys()) and not self.paramDef['UseVelo']:
            self.paramDef['UseVelo'] = True

        # set all params
        for prop, value in list(self.paramDef.items()):
        ...
        TrackCuts = {}
        if self.paramDef['UseVelo'] and not "Velo" in list(
                self.TrackSelector.keys()):
            self.TrackSelector["Velo"] = self.TrackVelo

        for trtype in list(self.TrackSelector.keys()):
            if trtype == "Long":
                TrackCuts[trtype] = {
    ...
    def _fixMCDATAConfig():
        ...
        if (PhysConf().getProp("Simulation")):
            ...
            for c in list(allConfigurables.values()):
