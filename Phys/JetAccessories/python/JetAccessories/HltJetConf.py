...
class HltParticleFlowConf(object):
        ...
        **kwargs
        ...
        if self.isRun2:
            if self.isMC == False:
                ...
                self.params["EcalElcEStr"] = (
                    "(-1.84500e-01 )*exp(-1.70173e-04*x)+8.10771e-01"
                )
                # ECAL energy charged hadron response:
                self.params["EcalHadEStr"] = (
                    "( 3.00834e-01 )*exp(-2.32541e-04*x)+3.49919e-01"
                )
                # HCAL energy electron response:
                self.params["HcalElcEStr"] = (
                    "(-1.84500e-01 )*exp(-1.70173e-04*x)+8.10771e-01"
                )
                # HCAL energy charged hadron :
                self.params["HcalHadEStr"] = (
                    "(1.72514e-01)*exp(-2.56450e-05*x)+6.11010e-01"
                )
                # ECAL + HCAL energy electron response:
                self.params["EcalHcalElcEStr"] = (
                    "(-1.84500e-01 )*exp(-1.70173e-04*x)+8.10771e-01"
                )
                # ECAL + HCAL energy charged hadron response:
                self.params["EcalHcalHadEStr"] = (
                    "( -2.15730e-01 +3.22409e-04*x +1.98637e-13*x*x)*exp(-2.16718e-04*x)+8.37867e-01"
                )
                # Function strings for Calo response in MCdata
            else:
                # ECAL energy electron response:
                self.params["EcalElcEStr"] = (
                    "( -2.86166e-01 )*exp(-6.44601e-05*x)+1.00566e+00"
                )
                # ECAL energy charged hadron response:
                self.params["EcalHadEStr"] = (
                    "( 1.49825e+00 )*exp(-4.16364e-04*x)+2.40328e-01"
                )
                # HCAL energy electron response:
                self.params["HcalElcEStr"] = (
                    "( -2.86166e-01 )*exp(-6.44601e-05*x)+1.00566e+00"
                ...
                # HCAL energy charged hadron:
                self.params["HcalHadEStr"] = (
                    "(1.36066e-01)*exp(-1.69589e-05*x)+6.45822e-01"
                )
                # ECAL + HCAL energy electron response:
                self.params["EcalHcalElcEStr"] = (
                    "( -2.86166e-01 )*exp(-6.44601e-05*x)+1.00566e+00"
                ...
                # ECAL + HCAL energy charged hadron response:
                self.params["EcalHcalHadEStr"] = (
                    "( -8.64325e-02 +3.03866e-05*x +7.45740e-14*x*x)*exp(-9.57188e-05*x)+7.85786e-01"
                )
        ...
        self.prepared_inputs = {
            "HFmesons": ["Particle", "particle", self.HFLocation],
        ...
        if self.useTurbo == False:
            self.inputs = _inputs
        else:
            self.inputs = _turboInputs
        self.seq = GaudiSequencer(_name + "ParticleFlowSeq", IgnoreFilterPassed=True)
            ...
            if key in list(self.prepared_inputs.keys()):
                ...
                # for common particles add all locations found as separate inputs
                    ...
                    for l in loc.locations:
                        ...
                        if "LD" not in l:
                            self.pflow.Inputs.append([inputClass, inputType, l])
    ...
    def _hcalclusters(self):
        ...
        clusTool = hcalClus.addTool(CaloClusterizationTool, "PFCaloClusterization")
...
class HltJetBuilderConf(object):
    ...
    def _configure(self):
        ...
        if self.jetid == False:
            builder_name += "_nojetid"
        else:
            builder_name += "_withjetid"
        ...
        if self.jetid:
            ...
            # jetfilter.Code = "(INFO(LHCb.JetIDInfo.CPF,-1) > 0.0) & (INFO(LHCb.JetIDInfo.MPT,-1)>100) & (max(INFO(LHCb.JetIDInfo.MTF,-1),INFO(LHCb.JetIDInfo.MNF,-1))<1.0) & (INFO(LHCb.JetIDInfo.Ntracks,-1) > 0)"
