...
public:
   ...
   */
  UpdateJetsWithVtx( const std::string& name, ISvcLocator* pSvc ) : GaudiAlgorithm( name, pSvc ), m_pvRelator( 0 ) {
    ...
    declareProperty( "PVRelatorName", m_pvRelatorName = DaVinci::DefaultTools::PVRelator );
  }
  // ========================================================================
...
DECLARE_COMPONENT( UpdateJetsWithVtx )
// ===========================================================================
  ...
  if ( type == 1 ) {
    ...
    for ( LHCb::Particles::const_iterator secjet = myJets->begin(); myJets->end() != secjet; secjet++ ) {
      ...
      for ( Parts::iterator idaug_secjet = daug_secjet.begin(); daug_secjet.end() != idaug_secjet; idaug_secjet++ ) {
        ...
        if ( track_second->type() != LHCb::Track::Types::Downstream ) { nonDown += E( *idaug_secjet ); }
      }
        ...
        for ( Parts::iterator idaug_secjet = daug_secjet.begin(); daug_secjet.end() != idaug_secjet; idaug_secjet++ ) {
          ...
          for ( Parts::iterator idaug_primjet = daug_primjet.begin(); daug_primjet.end() != idaug_primjet;
            ...
            for ( LHCb::Track::LHCbIDContainer::const_iterator i_idsFirst = idsFirst.begin();
              ...
              for ( LHCb::Track::LHCbIDContainer::const_iterator i_idsSec = idsSec.begin(); idsSec.end() != i_idsSec;
                    i_idsSec++ ) {
                if ( ( *i_idsFirst ) == ( *i_idsSec ) ) sharedID += 1.;
              }
            }
            if ( idsFirst.size() < idsSec.size() )
              sharedID = sharedID / idsFirst.size();
            else
              sharedID = sharedID / idsSec.size();
    ...
    for ( int i = 0; i < (int)jetList.size(); i++ ) {
      if ( jetList[i][2] > -0.5 ) {
        ...
        myJets->object( jetList[i][0] )
            ->addInfo( 54, jetList[i][1] * E( myInput->object( jetList[i][2] ) ) / jetList[i][3] );
        ...
        for ( Parts::iterator idaug_secjet = daug_secjet.begin(); daug_secjet.end() != idaug_secjet; idaug_secjet++ ) {
          ...
          if ( track->type() != LHCb::Track::Types::Downstream ) {
            ...
            const LHCb::VertexBase* vtx =
                m_pvRelator->relatedPV( *idaug_secjet, LHCb::VertexBase::ConstVector( PVS->begin(), PVS->end() ) );
