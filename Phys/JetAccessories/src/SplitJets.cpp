...
DECLARE_COMPONENT( SplitJets )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
SplitJets::SplitJets( const std::string& name, ISvcLocator* pSvcLocator )
  ...
  declareProperty( "PseudoJetID", m_pseudoJetID, "Particle ID for the PseudoJet" );
  declareProperty( "reSetMomentumAndKeys", m_reSetMomentumAndKeys,
                   "re set the extrainfo keys of the jet and its corrected momentum" );
}
...
StatusCode SplitJets::execute() {
  ...
  for ( LHCb::Particle::Range::iterator ip = jets.begin(); ip != jets.end(); ip++ ) {
    ...
    for ( LHCb::Particle::ConstVector::const_iterator idaug = jetDaughters.begin(); jetDaughters.end() != idaug;
          ++idaug ) {
      if ( ( *idaug )->particleID().hasBottom() )
        theB = ( *idaug );
      else
        newDaugs.push_back( ( *idaug ) );
    }
    ...
    if ( theB->particleID().pid() < 0 ) tempID = -m_pseudoJetID;
    ...
    int tempID2 = m_jetID;
    if ( theB->particleID().pid() < 0 ) tempID2 = -m_jetID;
    ...
    if ( m_reSetMomentumAndKeys ) {
      ...
      for ( int i = 0; i < 10000; i++ )
        if ( p->hasInfo( i ) ) pJet.addInfo( i, p->info( i, -999.0 ) );
    }
