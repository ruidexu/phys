...
StatusCode HltParticleFlow::initialize() {
  ...
  if ( m_sprRecover ) {
    ...
    if ( !m_ecalElcE.IsValid() ) return Error( "Invalid ECAL energy electron response function." );
    if ( !m_ecalHadE.IsValid() ) return Error( "Invalid ECAL energy charged hadron response function." );
    if ( !m_hcalElcE.IsValid() ) return Error( "Invalid HCAL energy electron response function." );
    if ( !m_hcalHadE.IsValid() ) return Error( "Invalid HCAL energy charged hadron response function." );
    if ( !m_ecalHcalElcE.IsValid() ) return Error( "Invalid HCAL + ECAL energy electron response function." );
  ...
  for ( unsigned int best = 0; best < m_proBestNames.size(); ++best ) {
    ...
    if ( !prp ) return Error( "Unknown particle name '" + m_proBestNames[best] + "' in ProBestNames." );
  ...
  for ( unsigned int inLoc = 0; inLoc < m_inLocs.size(); ++inLoc ) {
    Input in( &inLoc, &m_inLocs[inLoc], m_prtSvc );
    if ( in.valid && in.name != "RecVertex" && in.name != "IClusTrTable2D" ) in.print( verbose() );
  }
...
StatusCode HltParticleFlow::execute() {
  ...
  m_trksEcal = 0;
  m_trksHcal = 0;
  if ( m_inEcalTrks.valid && retrieve( m_ecalTrks, m_inEcalTrks ) ) m_trksEcal = m_ecalTrks->inverse();
  if ( m_inHcalTrks.valid && retrieve( m_hcalTrks, m_inHcalTrks ) ) m_trksHcal = m_hcalTrks->inverse();
  ...
  for ( unsigned int inLoc = 0; inLoc < m_inLocs.size(); ++inLoc ) {
    ...
    if ( inPro != m_inPros.end() && inLoc == inPro->index ) {
      ...
      if ( exist<ProtoParticles>( inPro->loc ) ) {
        ...
        for ( ProtoParticles::iterator pro = pros->begin(); pro != pros->end(); ++pro ) add( *pro, *inPro );
      } else {
  ...
  if ( m_sprRecover ) {
    if ( msgLevel( MSG::DEBUG ) ) { debug() << "Adding SuperClusters from neutral recovery." << endmsg; }
...
void HltParticleFlow::add( const Particle* prt, const Input& in ) {
  ...
  if ( in.ban ) {
    for ( Particle::ConstVector::iterator dtr = dtrs.begin(); dtr != dtrs.end(); ++dtr ) use( ( *dtr )->proto(), 0 );
    return;
  }
  for ( Particle::ConstVector::iterator dtr = dtrs.begin(); dtr != dtrs.end(); ++dtr )
    if ( used( ( *dtr )->proto() ) ) return;
  for ( Particle::ConstVector::iterator dtr = dtrs.begin(); dtr != dtrs.end(); ++dtr ) {
    if ( in.dtrs ) create( *dtr );
...
Particle* HltParticleFlow::create( const Particle* prt ) {
  ...
  if ( m_inNow ) out->addInfo( Particle::additionalInfo::FirstJetIndex, m_inNow->index );
  m_prts->insert( out );
  return out;
}
...
Particle* HltParticleFlow::create( const ProtoParticle* pro, const int& id, const double& m ) {
  ...
  if ( pro->track() ) {
    ...
    for ( CaloHypo::Clusters::const_iterator cal = cals.begin(); cal != cals.end(); ++cal ) sum( vec, *cal );
  ...
  prt->setProto( pro );
  if ( m_inNow ) prt->addInfo( Particle::additionalInfo::FirstJetIndex, m_inNow->index );
  m_prts->insert( prt );
  return prt;
}
...
void HltParticleFlow::use( const Track* trk, const Particle* prt ) {
  m_trks.insert( trk );
  if ( trk->type() == Track::Types::Velo ) return;
  SuperClusterPtr      spr( 0 );
  Gaudi::LorentzVector vec( 0, 0, 0, 0 );
  if ( prt ) spr = SuperClusterPtr( new SuperCluster() );
  if ( m_trksEcal ) use( trk, vec, spr, spr ? &spr->ecal : 0, m_trksEcal, m_ecalBest, m_ecalChi2Max );
  if ( m_trksHcal ) use( trk, vec, spr, spr ? &spr->hcal : 0, m_trksHcal, m_hcalBest, m_hcalChi2Max );
