...
DECLARE_COMPONENT(HltParticleFlow)
    ...
    : GaudiHistoAlg(name, svc), m_prtSvc(0), m_prtSta(0), m_prtVrt(0), m_inNow(0)
{
  ...
  declareProperty("Inputs", m_inLocs,
                  ...
                  "<location>].");
  declareProperty("Output", m_outLoc = "Phys/ParticleFlow/Particles", "PF output Particle location.");
  ...
  declareProperty("TrkVlP", m_trkVlP = 1 * Gaudi::Units::MeV, "Momentum to assign VELO Tracks.");
  ...
  declareProperty("TrkLnErrMax", m_trkLnErrMax = -1,
                  "Fix momentum for long Tracks with curvature / error "
                  "below this.");
  declareProperty("TrkUpErrMax", m_trkUpErrMax = -1,
                  "Fix momentum for upstream Tracks with curvature / error "
                  "below this.");
  ...
  declareProperty("SprPid", m_sprPid = 22, "ID to assign SuperClusters from neutral recovery.");
  declareProperty("SprM", m_sprM = 0 * Gaudi::Units::MeV, "Mass to assign SuperClusters from neutral recovery.");
  declareProperty("EcalBest", m_ecalBest = true, "Flag to only match the best ECAL CaloCluster for a Track.");
  declareProperty("HcalBest", m_hcalBest = true, "Flag to only match the best HCAL CaloCluster for a Track.");
  ...
  declareProperty("HcalChi2Max", m_hcalChi2Max = 25,
                  ...
                  "(-1 is +inf).");
  declareProperty("EcalElcEStr", m_ecalElcEStr = "(-2.87675e-01)*exp(-1.89536e-04*x) + 8.86869e-01",
                  "Function string for the ECAL energy electron response.");
  declareProperty("EcalHadEStr", m_ecalHadEStr = "(-3.79500e-01)*exp(-1.11353e-04*x) + 8.38989e-01",
                  "Function string for the ECAL energy charged hadron "
                  "response.");
  declareProperty("HcalElcEStr", m_hcalElcEStr = "(-2.87675e-01)*exp(-1.89536e-04*x) + 8.86869e-01",
                  "Function string for the HCAL energy electron response.");
  declareProperty("HcalHadEStr", m_hcalHadEStr = "(2.21942e-01)*exp(-7.76006e-05*x) + 3.51139e-01",
                  "Function string for the HCAL energy charged hadron "
                  "response.");
  declareProperty("EcalHcalElcEStr", m_ecalHcalElcEStr = "(-2.87675e-01)*exp(-1.89536e-04*x) + 8.86869e-01",
                  "Function string for the ECAL + HCAL energy electron "
                  "response.");
  declareProperty("EcalHcalHadEStr",
                  m_ecalHcalHadEStr = "(-7.48397e-02 - "
                                      "1.11391e-05*x - 2.51717e-09*x*x)*exp(-1.18558e-04*x) + "
                                      "7.76469e-01",
                  "Function string for the ECAL + HCAL energy charged hadron "
                  "response.");
  declareProperty("TieBreaker", m_tiebreak = 0, "Option to choose between multiple candidates ");
  declareProperty("HFLocation", m_HFlocation = "", "HF location");
}
...
StatusCode HltParticleFlow::initialize()
{
  ...
  if (!m_prtSta)
    m_prtSta = tool<IParticle2State>("Particle2State", this);
  if (!m_prtSta)
    return Error("Could not retrieve the Particle2State tool.");
  ...
  for (unsigned int inLoc = 0; inLoc < m_inLocs.size(); ++inLoc)
  {
    ...
    else if (in.name == "ProtoParticle")
      m_inPros.push_back(in);
    else if (in.name == "Track")
      m_inTrks.push_back(in);
    else if (in.name == "CaloCluster")
      m_inCals.push_back(in);
    else if (in.name == "RecVertex")
    {
      if (!m_inVrts.valid)
        m_inVrts = in;
      else
        warning() << "RecVertex location already specified, skipping input "
                     " element "
                  << inLoc << "." << endmsg;
    }
    else if (in.name == "IClusTrTable2D")
    {
      if (in.ecal)
      {
        if (!m_inEcalTrks.valid)
          m_inEcalTrks = in;
        else
          warning() << "ECAL IClusTrTable2D location already specified, "
                       "skipping input element "
                    << inLoc << "." << endmsg;
      }
      else
      {
        if (!m_inHcalTrks.valid)
          m_inHcalTrks = in;
        else
          warning() << "HCAL IClusTrTable2D location already specified, "
                       "skipping input element "
                    << inLoc << "." << endmsg;
      }
    }
  ...
  {
    if (!m_prtVrt)
      m_prtVrt = tool<IRelatedPVFinder>("GenericParticle2PVRelator<_p2PVWithIPChi2, "
                                        "OfflineDistanceCalculatorName>/P2PVWithIPChi2",
                                        this);
    if (!m_prtVrt)
      return Error("Could not retrieve RelatedPVFinder tool.");
  }
  ...
  m_proBestMs.clear();
  m_proBestPids.clear();
  if (m_proBestNames.size() != m_proBestKeys.size() || m_proBestNames.size() != m_proBestMins.size())
    return Error("The ProBestNames, ProBestKeys, and ProBestMins lists must "
                 "have the same size.");
  if (!m_proBestNames.size())
  {
    m_proBestNames.push_back("pi+");
    m_proBestMins.push_back(-1);
    m_proBestKeys.push_back(ProtoParticle::additionalInfo::ProbNNpi);
  }
  for (unsigned int best = 0; best < m_proBestNames.size(); ++best)
  {
    const ParticleProperty *prp = m_prtSvc->find(m_proBestNames[best]);
    if (!prp)
      return Error("Unknown particle name '" + m_proBestNames[best] + "' in ProBestNames.");
  ...
  if (m_inVrts.valid)
    m_inVrts.print(verbose(), false);
  if (m_inEcalTrks.valid)
    m_inEcalTrks.print(verbose(), false);
  if (m_inHcalTrks.valid)
    m_inHcalTrks.print(verbose(), false);
  verbose() << "\nProtoParticle best PIDs:\n"
            << setw(25) << "particle name" << setw(25) << "extra info key" << setw(25) << "extra info minimum"
            << "\n";
  for (unsigned int best = 0; best < m_proBestNames.size(); ++best)
    verbose() << setw(25) << m_proBestNames[best] << setw(25) << m_proBestKeys[best] << setw(25)
              << m_proBestMins[best] << "\n";
  verbose() << endmsg;
...
StatusCode HltParticleFlow::execute()
{
  ...
  m_trks.clear();
  m_calsSprs.clear();
  m_prts = new Particles();
  m_pros = new ProtoParticles();
  m_hyps = new CaloHypos();
  m_cals = new CaloClusters();
  put(m_prts, m_outLoc);
  put(m_pros, m_outPre + "/ProtoParticles");
  put(m_hyps, m_outPre + "/CaloHypos");
  put(m_cals, m_outPre + "/CaloClusters");
  if (m_prtVrt)
  {
    m_prtsVrts = new Particle2Vertex::WTable();
    put(m_prtsVrts, m_outPre + "/Particle2VertexRelations");
  }
  ...
  // Add the inputs.
  vector<Input>::iterator inPrt(m_inPrts.begin()), inPro(m_inPros.begin()), inTrk(m_inTrks.begin()),
      inCal(m_inCals.begin());
  for (unsigned int inLoc = 0; inLoc < m_inLocs.size(); ++inLoc)
  {
    // if (abs(inPrt->pid) == 521)
    // std::cout << inPrt->pid << ", ";
    if ((m_tiebreak == 1) && inPrt->loc == m_HFlocation)
    {
      vector<const Particle *> sorted_prts;
      if (inPrt != m_inPrts.end() && inLoc == inPrt->index)
      {
        m_inNow = &(*inPrt);
        if (exist<typename Gaudi::Range_<vector<const Particle *>>>(inPrt->loc))
        {
          Gaudi::Range_<vector<const Particle *>> objs = get<typename Gaudi::Range_<vector<const Particle *>>>(inPrt->loc);
          for (typename Gaudi::Range_<vector<const Particle *>>::iterator obj = objs.begin(); obj != objs.end(); ++obj)
          {
            sorted_prts.push_back(*obj);
          }
          std::sort(sorted_prts.begin(), sorted_prts.end(), sort_rule());
          for (vector<const Particle *>::iterator prt = sorted_prts.begin(); prt != sorted_prts.end(); ++prt)
          {
            add(*prt, *inPrt);
          }
        }
        else
        {
          ...
          {
            debug() << "add(): Could not retrieve " << inPrt->name << "s from " << inPrt->loc << endmsg;
          }
        }
        ++inPrt;
        continue;
    ...
    {
      if (add<Particle>(&m_inPrts, inPrt, inLoc))
        continue;
    }
    ...
    if (inPro != m_inPros.end() && inLoc == inPro->index)
    {
      ...
      if (exist<ProtoParticles>(inPro->loc))
      {
        ...
        for (ProtoParticles::iterator pro = pros->begin(); pro != pros->end(); ++pro)
          add(*pro, *inPro);
      }
      else
      {
        Warning("execute(): Could not retrieve " + inPro->name + " from " + inPro->loc).ignore();
      }
      ++inPro;
      continue;
    }
    if (add<Track>(&m_inTrks, inTrk, inLoc))
      continue;
    if (add<CaloCluster>(&m_inCals, inCal, inLoc))
      continue;
  }
  ...
  if (m_sprRecover)
  {
    ...
    for (unordered_map<const CaloCluster *, SuperClusterPtr>::iterator calSpr = m_calsSprs.begin();
         calSpr != m_calsSprs.end(); ++calSpr)
    {
      if (!calSpr->second)
        continue;
...
//=============================================================================
// Constructor.
HltParticleFlow::Input::Input(const unsigned int *idx, const vector<string> *prp, const IParticlePropertySvc *svc)
    : valid(false), ban(false), dtrs(false), ecal(false), pid(0), m(0)
{
  ...
  if (!svc)
  {
    warn += "invalid pointer to particle service.";
    return;
  }
  if (prp->size() != 3)
  {
  ...
  if (s0 != "RecVertex" && s0 != "IClusTrTable2D" && s1 == "ban")
    ban = true;
  else if (s0 == "Particle" && s1 == "particle")
    dtrs = false;
  else if (s0 == "Particle" && s1 == "daughters")
    dtrs = true;
  else if (s0 == "ProtoParticle" && s1 == "best")
    m = -1;
  else if (s0 == "IClusTrTable2D" && s1 == "ecal")
    ecal = true;
  else if (s0 == "IClusTrTable2D" && s1 == "hcal")
    ecal = false;
  ...
  else if (s0 != "Particle")
  {
    ...
    if (!prp)
    {
      warn += "unknown particle name " + s1 + ".";
      return;
    }
    m = prp->mass();
    pid = prp->pid().pid() * (prp->charge() >= 0 ? 1 : -1);
  }
  else
  {
...
void HltParticleFlow::add(const Particle *prt, const Input &in)
{
  ...
  for (Particle::ConstVector::iterator dtr = dtrs.begin(); dtr != dtrs.end(); ++dtr)
  {
    if (in.dtrs)
      create(*dtr);
    use((*dtr)->proto(), m_sprRecover ? *dtr : 0);
  }
  if (!in.dtrs)
    create(prt);
}
...
Particle *HltParticleFlow::create(const Particle *prt)
{
  ...
  if (m_inNow)
    out->addInfo(Particle::additionalInfo::FirstJetIndex, m_inNow->index);
  m_prts->insert(out);
  return out;
}

// Create a Particle from a ProtoParticle to use in the PF output Particles.
Particle *HltParticleFlow::create(const ProtoParticle *pro, const int &id, const double &m)
{
  Particle *prt(0);
  ...
  if (pro->track())
  {
    ...
    {
      double c(abs(sta.qOverP())), e(sqrt(sta.errQOverP2()));
      if (e != 0)
        ;
      else if (type == Track::Types::Long && c / e < m_trkLnErrMax)
        p = m_trkLnP;
      else if (type == Track::Types::Upstream && c / e < m_trkUpErrMax)
        p = m_trkUpP;
      else if (type == Track::Types::Downstream && c / e < m_trkDnErrMax)
        p = m_trkDnP;
    }
    if (p > 0)
      sta.setQOverP(q / p);
    if (m_prtSta->state2Particle(sta, *prt).isFailure())
    {
      delete prt;
      return 0;
    }
  ...
  vec.SetE(sqrt(vec.P2() + m * m));
  prt->setMomentum(vec);
  prt->setMeasuredMass(m);
  prt->setMeasuredMassErr(0);
  prt->setProto(pro);
  if (m_inNow)
    prt->addInfo(Particle::additionalInfo::FirstJetIndex, m_inNow->index);
  m_prts->insert(prt);
  return prt;
}

// Create a Particle from a Track to use in the PF output Particles.
Particle *HltParticleFlow::create(const Track *trk, const int &id, const double &m)
{
...
Particle *HltParticleFlow::create(const CaloCluster *cal, const int &id, const double &m)
{
...
{
  CaloPosition pos;
  pos.setParameters(Gaudi::Vector3(spr->vec.X() / spr->vec.E(), spr->vec.Y() / spr->vec.E(), spr->vec.E()));
  ...
  if (m_inNow)
  {
    prt->eraseInfo(Particle::additionalInfo::FirstJetIndex);
    prt->addInfo(Particle::additionalInfo::FirstJetIndex, m_inNow->index + 1);
  }
  m_cals->insert(cal);
  return prt;
}
...
{
  m_trks.insert(trk);
  if (trk->type() == Track::Types::Velo)
    return;
  SuperClusterPtr spr(0);
  Gaudi::LorentzVector vec(0, 0, 0, 0);
  if (prt)
    spr = SuperClusterPtr(new SuperCluster());
  if (m_trksEcal)
    use(trk, vec, spr, spr ? &spr->ecal : 0, m_trksEcal, m_ecalBest, m_ecalChi2Max);
  if (m_trksHcal)
    use(trk, vec, spr, spr ? &spr->hcal : 0, m_trksHcal, m_hcalBest, m_hcalChi2Max);

  // Determine the expected Particle energy and update the SuperCluster.
  if (!prt || (!spr->ecal && !spr->hcal))
    return;
  double e(0), p(prt->p());
  if (prt->particleID().abspid() == 11)
  {
    if (spr->ecal && spr->hcal)
      e += p * m_ecalHcalElcE.Eval(p);
    else if (spr->hcal)
      e += p * m_hcalElcE.Eval(p);
    else
      e += p * m_ecalElcE.Eval(p);
  }
  else if (prt->charge() != 0 && prt->particleID().isHadron())
  {
    if (spr->ecal && spr->hcal)
      e += p * m_ecalHcalHadE.Eval(p);
    else if (spr->hcal)
      e += p * m_hcalHadE.Eval(p);
    else
      e += p * m_ecalHadE.Eval(p);
  }
...
void HltParticleFlow::use(const Track *trk, Gaudi::LorentzVector &vec, SuperClusterPtr &spr, bool *calo,
                          const Calo2Track::ITrClusTable *trksCals, const bool &best, const double &chi2Max)
{
  ...
  if (cals.size() == 0)
    return;
  for (Calo2Track::ITrClusTable::Range::const_iterator cal = cals.begin(); cal != cals.end(); ++cal)
  {
    if (chi2Max >= 0 && cal->weight() > chi2Max)
      continue;
    if (calo)
    {
      sum(vec, cal->to());
...
{
  unordered_map<const CaloCluster *, SuperClusterPtr>::iterator calSpr = m_calsSprs.find(cal);
  ...
  if (calSpr->second)
  {
    while (calSpr->second->spr)
      calSpr->second = calSpr->second->spr;
    if (spr != calSpr->second)
    {
      spr->vec += calSpr->second->vec;
      calSpr->second->used = true;
...
bool HltParticleFlow::used(const Track *trk) { return m_trks.find(trk) != m_trks.end(); }

// Check if a CaloCluster is used.
bool HltParticleFlow::used(const CaloCluster *cal) { return m_calsSprs.find(cal) != m_calsSprs.end(); }
...
void HltParticleFlow::sum(Gaudi::LorentzVector &vec, const CaloCluster *cal)
{
  if (!cal)
  {
    Warning("sum(): Invalid CaloCluster passed").ignore();
    return;
  }
...
{
  const Particle2Vertex::LightWTable table = m_prtVrt->relatedPVs(prt, m_vrts);
