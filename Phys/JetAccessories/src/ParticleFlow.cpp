...
DECLARE_COMPONENT( ParticleFlow )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
ParticleFlow::ParticleFlow( const std::string& name, ISvcLocator* pSvcLocator )
  ...
  declareProperty( "UseVelo", m_useVelo = false, "Use Velo tracks turning them into 1*eV mommentum particles" );
  ...
  declareProperty( "MinPhotonIDMin4ResolvedPi0", m_photonIDMin4ResolvedPi0 = -2.,
                   "Minimum photon pid for photon from resolved Pi0" );
  declareProperty( "MinHCALE", m_minHCALE = 0., "Minimum value to take a HCAL cluster into account" );
  declareProperty( "MinHCALEt", m_minHCALEt = 2500., "Minimum Et value to take a HCAL cluster into account" );
  declareProperty( "MinHCALEt4Eta4", m_minHCALEt4Eta4 = 5000.,
                   "Minimum Et value to take a HCAL cluster into account for large eta > 4" );
  declareProperty( "MinIsoPhotonEt", m_minIsoPhotonEt = 2500., "Minimum Et value an isolated photon into account" );
  declareProperty( "MinPhotonEt", m_minPhotonEt = 400., "Minimum Et value an photon into account" );
  declareProperty( "MinBadPhotonEt", m_minBadPhotonEt = 2500., "Minimum Et value an photon into account" );
  ...
  // TODO should be ET
  declareProperty( "MinE", m_MinE = 2000., "Minimal energy for a hidden neutral to be recovered" );
  declareProperty( "MinEt", m_MinEt = 2500., "Minimal transverse energy for a hidden neutral to be recovered" );
  ...
  declareProperty( "MC_recovery", m_MC = true, "Is it MC? Needed for the Calo E response" );
...
StatusCode ParticleFlow::initialize() {
  ...
  m_protoMap["pi+"] =
      std::make_pair( tool<IProtoParticleFilter>( "ProtoParticleANNPIDFilter", "pion", this ), m_ppSvc->find( "pi+" ) );
  m_protoMap["K+"] =
      std::make_pair( tool<IProtoParticleFilter>( "ProtoParticleANNPIDFilter", "kaon", this ), m_ppSvc->find( "K+" ) );
  ...
  m_protoMap["e+"] = std::make_pair( tool<IProtoParticleFilter>( "ProtoParticleANNPIDFilter", "electron", this ),
                                     m_ppSvc->find( "e+" ) );
  m_protoMap["mu+"] =
      std::make_pair( tool<IProtoParticleFilter>( "ProtoParticleMUONFilter", "muon", this ), m_ppSvc->find( "mu+" ) );
  ...
  m_ECALHCALHadron_Data = new TF1( "ECALHCALHadron_Data",
                                   "(-7.48397e-02 -1.11391e-05*x -2.51717e-09*x*x)*exp(-1.18558e-04*x)+7.76469e-01" );
  m_ECALHCALHadron_MC =
      new TF1( "ECALHCALHadron_MC", "( 1.45913e-01 -6.70718e-05*x +1.34026e-09*x*x)*exp(-9.39504e-05*x)+8.10820e-01" );
  ...
  // Note: Run II configuration is different, check HLT jets configuration
...
StatusCode ParticleFlow::execute() {
  ...
  if ( !this->loadDatas() ) { return Warning( "Could not load datas" ); }
  ...
  if ( m_protoParticles.count( "Charged" ) > 0.5 ) {
    ...
    treatProtoParticles( "Charged", PFParticles ); //, *table );
  }
  ...
  if ( m_protoParticles.count( "Velo" ) > 0.5 ) {
    ...
    treatProtoParticles( "Velo", PFParticles ); // , *table );
  }
  ...
  if ( m_particleContainers.count( "MergedPi0" ) > 0.5 ) {
    ...
    treatPi0s( LHCb::PFParticle::MergedPi0, PFParticles ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
  }
  ...
  if ( m_particleContainers.count( "ResolvedPi0" ) > 0.5 ) {
    ...
    treatPi0s( LHCb::PFParticle::ResolvedPi0, PFParticles ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
  }
  ...
  if ( m_particleContainers.count( "Photons" ) > 0.5 ) {
    ...
    treatPhotons( PFParticles ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
  }
  ...
  if ( m_useHCAL ) {
    ...
    treatNeutralHadronsAndIsolatedPhotons( PFParticles ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
  }
  ...
  if ( m_neutralRecovery ) {
    ...
    treatNeutralRecovery( PFParticles ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
  }
  ...
  for ( std::vector<LHCb::PFParticle*>::const_iterator i_pfp = PFParticles.begin(); PFParticles.end() != i_pfp;
        ++i_pfp ) {
    LHCb::Particle* newPart = new LHCb::Particle( *( *i_pfp ) );
    verbose() << "Extrainfo: " << newPart->info( 900, -1. ) << endmsg;
    if ( ( newPart->info( 900, -1. ) > 1. && newPart->info( 900, -1. ) < 10. ) ||
         ( newPart->info( 900, -1. ) > 100. && newPart->info( 900, -1. ) < 1000. ) ) {
  ...
  // MONITORING: check all cluster are used, check there is no double counting between tracks, and between neutral calo
  // clusters and neutral recovery clusters Delete the PFCaloClusters
...
StatusCode ParticleFlow::updatePFCaloClusters( LHCb::PFParticle& theParticle, unsigned int seedID ) {
  ...
  if ( ( theParticle.PFType() > LHCb::PFParticle::Charged && theParticle.PFType() < LHCb::PFParticle::Neutral ) ||
       ...
       ( theParticle.PFType() == LHCb::PFParticle::Daughter &&
         ( ( theParticle.PFDaugType() > LHCb::PFParticle::Charged &&
             theParticle.PFDaugType() < LHCb::PFParticle::Neutral ) ||
           theParticle.PFType() == LHCb::PFParticle::ChargedInfMomentum ) ) ) {

    if ( theParticle.proto() == NULL || theParticle.proto()->track() == NULL ) {
      return Warning( "this is not a proper charged particle", StatusCode::FAILURE );
    }
  ...
  // Case of neutral particles
  else if ( ( theParticle.PFType() > LHCb::PFParticle::Neutral &&
              theParticle.PFType() < LHCb::PFParticle::Composite ) ||
            theParticle.PFType() == LHCb::PFParticle::BadPhotonMatchingT ||
            theParticle.PFType() == LHCb::PFParticle::BadPhoton ||
            theParticle.PFType() == LHCb::PFParticle::IsolatedPhoton ) {
    ...
    // m_PFCaloClusters[theParticle.proto()->calo()[0]->clusters
    // ()[0].target()->seed().all()]->updateParticleList(&theParticle); if(theParticle.PFType() !=
    // LHCb::PFParticle::BadPhoton)
    if ( theParticle.PFType() == LHCb::PFParticle::IsolatedPhoton ||
         theParticle.PFType() == LHCb::PFParticle::NeutralHadron ) {
    ...
    m_PFCaloClusters[theParticle.proto()->calo()[0]->clusters()[0].target()->seed().all()]->setStatus(
        LHCb::PFCaloCluster::Used );

    verbose() << "tag UPF n 7" << endmsg;

    return StatusCode::SUCCESS;
  } else if ( theParticle.PFType() == LHCb::PFParticle::additionalInfo::Unknown ) {
    verbose() << theParticle << endmsg;
    verbose() << theParticle.proto() << endmsg;
    verbose() << theParticle.proto()->track() << endmsg;
    return Warning( "Unknown type of particle in updatePFCaloClusters", StatusCode::FAILURE );
  }
  return Warning( "No implementation for this type of particle in updatePFCaloClusters", StatusCode::FAILURE );
}
...
int ParticleFlow::tagTrack( const LHCb::Track* track ) {
  ...
  if ( m_trSel != 0 ) {
    ...
    verbose() << "Track " << track->type() << " with pt: " << track->pt() << " chi2 per dof " << track->chi2PerDoF()
              << endmsg;
  ...
  if ( track->type() == LHCb::Track::Types::Long || track->type() == LHCb::Track::Types::Downstream ) {
    ...
    if ( ( track->type() == LHCb::Track::Types::Long &&
           std::fabs( firstS.qOverP() / sqrt( firstS.errQOverP2() ) ) < m_cutInfMomTRVal ) ||
         ( track->type() == LHCb::Track::Types::Downstream &&
           std::fabs( firstS.qOverP() / sqrt( firstS.errQOverP2() ) ) < m_cutInfMomTRVal_Down ) ) {
      return LHCb::PFParticle::KeepInfMom;
    }
  }
  // Tag upstream tracks with large curvature errors.
  else if ( track->type() == LHCb::Track::Types::Upstream ) {
    const LHCb::State& firstS = track->firstState();
  ...
  else if ( track->type() == LHCb::Track::Types::Velo ) {
    return LHCb::PFParticle::TurnTo0Momentum;
  }
...
StatusCode ParticleFlow::loadDatas() {
   ...
   *
   * Load standard particles that needs corrections ( electrons, pi0s, photons).
  ...
  for ( std::vector<std::string>::const_iterator i_location = m_compositeParticleLocations.begin();
    ...
    if ( exist<LHCb::Particle::Range>( *i_location ) )
      m_particleContainers["Composite"].push_back( get<LHCb::Particle::Range>( *i_location ) );
  }
  ...
  for ( std::vector<std::string>::const_iterator i_location = m_keepCandidatesLocations.begin();
    ...
    if ( exist<LHCb::Particle::Range>( *i_location ) )
      m_particleContainers["ToKeep"].push_back( get<LHCb::Particle::Range>( *i_location ) );
  }
  ...
  for ( std::vector<std::string>::const_iterator i_location = m_banCandidatesLocations.begin();
    ...
    if ( exist<LHCb::Particle::Range>( *i_location ) )
      m_particleContainers["ToBan"].push_back( get<LHCb::Particle::Range>( *i_location ) );
  }
...
StatusCode ParticleFlow::preprocessDatas( std::vector<LHCb::PFParticle*>& PFParticles ) { //, Particle2Vertex::WTable&
                                                                                          // table ) {
  /**
    ...
    BOOST_FOREACH ( const LHCb::Particle* cand, particleToKeep ) {
      ...
      if ( particleToKeep_daug.size() == 0 ) {
        ...
        for ( LHCb::Particle::ConstVector::const_iterator i_p = particleToKeep_daug.begin();
          ...
          // IMPROVEMENT: get the calocluster and if chi2 is good put it to to NeutralRecovery with this particle
          // attached Not done yet because assume most of the time one anyway wants it to be isolated of the jet
        }
      }
    }
  }
  ...
  if ( m_particleContainers.count( "Composite" ) > 0.5 ) {
    ...
    BOOST_FOREACH ( const LHCb::Particle* comp, sortComposites ) {
      ...
      LoKi::Extract::getParticles( comp, std::back_inserter( daughters ),
                                   LoKi::Cuts::ID != motherID && LoKi::Cuts::HASPROTO );
      bool overlap = false;
      ...
      for ( LHCb::Particle::ConstVector::const_iterator i_p = daughters.begin(); daughters.end() != i_p; ++i_p ) {
        ...
        if ( ( *i_p )->proto() == NULL )
          return Error( "Composite daugthers of composite particles treatement is not implemented" );
        if ( ( *i_p )->proto()->track() == NULL )
          Error( "Neutral daugthers of composite particles treatement is not implemented" ).ignore();
      ...
      if ( !overlap ) {
        ...
        for ( LHCb::Particle::ConstVector::const_iterator i_p = daughters.begin(); daughters.end() != i_p; ++i_p ) {
          ...
          this->updatePFCaloClusters( *theDaugtherParticle ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
...
//=============================================================================
void ParticleFlow::relate2Vertex( const LHCb::Particle* p, Particle2Vertex::WTable& table ) {
  /**
...
//=============================================================================
void ParticleFlow::treatProtoParticles( std::string                     pptype,
                                        std::vector<LHCb::PFParticle*>& PFParticles ) { // , Particle2Vertex::WTable&
                                                                                        // table){
  /**
  ...
  BOOST_FOREACH ( const LHCb::ProtoParticle* ch_pp, *m_protoParticles[pptype] ) {
    ...
    int                           tag( LHCb::PFParticle::additionalInfo::Unknown );
    ...
    if ( !( m_trackKeyToKeep.count( track->key() ) ) ) {
      ...
      if ( pptype == "Charged" ) {
        double nNN = ch_pp->info( LHCb::ProtoParticle::additionalInfo::ProbNNghost, -1 );
    ...
    if ( theParticle == NULL )
      ...
      this->updatePFCaloClusters( *theParticle ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
...
StatusCode ParticleFlow::treatPi0s( int pftype, std::vector<LHCb::PFParticle*>& PFParticles ) {
  ...
  BOOST_FOREACH ( const LHCb::Particle* pi0, m_particleContainers[typeCont][0] ) {
    ...
    if ( pftype == LHCb::PFParticle::ResolvedPi0 ) {
      ...
      double minPhotonID = std::min( daugs[0]->proto()->info( LHCb::ProtoParticle::additionalInfo::PhotonID, -100. ),
                                     daugs[1]->proto()->info( LHCb::ProtoParticle::additionalInfo::PhotonID, -100. ) );
      double maxPhotonID = std::max( daugs[0]->proto()->info( LHCb::ProtoParticle::additionalInfo::PhotonID, -100. ),
                                     daugs[1]->proto()->info( LHCb::ProtoParticle::additionalInfo::PhotonID, -100. ) );
    ...
    int goodCluster = 0;
    int nSatCell( 0 );
    for ( LHCb::Particle::ConstVector::const_iterator i_daug = realPart.begin(); realPart.end() != i_daug; ++i_daug ) {
    ...
    if ( ( goodCluster < 2 && pftype == LHCb::PFParticle::ResolvedPi0 ) ||
         ( goodCluster < 1 && pftype == LHCb::PFParticle::MergedPi0 ) )
      continue;
...
StatusCode ParticleFlow::treatPhotons( std::vector<LHCb::PFParticle*>& PFParticles ) {
  ...
  BOOST_FOREACH ( const LHCb::Particle* photon, m_particleContainers["Photons"][0] ) {
    ...
    if ( !hypoClusters[0] ||
         m_PFCaloClusters[hypoClusters[0].target()->seed().all()]->status() != LHCb::PFCaloCluster::Available )
      continue;
    ...
    if ( m_banFromTTrack ) {
      ...
      if ( tRange.size() > 0 ) {
        if ( tRange.front().to()->type() == LHCb::Track::Types::Ttrack && tRange.front().weight() < m_Chi2ECALCutTT ) {
          // Check the PID
          int PFType = LHCb::PFParticle::additionalInfo::Unknown;
          if ( n_pp->info( LHCb::ProtoParticle::additionalInfo::PhotonID, -100. ) < m_photonID4PhotonTban ||
    ...
    if ( n_pp->info( LHCb::ProtoParticle::additionalInfo::PhotonID, -100. ) < m_photonID4Photon )
    ...
    theParticle->setNSaturatedCellsECAL( nSatCell );
    theParticle->setPFType( PFType );
    verbose() << "Type in photons: " << PFType << " " << theParticle->PFType() << " "
              << theParticle->info( 900, -1000. ) << endmsg;
    this->updatePFCaloClusters( *theParticle ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
    // Save the particle
    PFParticles.push_back( theParticle );
  }
...
// LHCb::CaloHypos& PFCaloHypos, LHCb::ProtoParticles& PFProtoParticles ){
StatusCode ParticleFlow::treatNeutralHadronsAndIsolatedPhotons( std::vector<LHCb::PFParticle*>& PFParticles ) {
  /**
  ...
  for ( std::map<unsigned int, LHCb::PFCaloCluster*>::iterator iclu = m_PFCaloClusters.begin();
        m_PFCaloClusters.end() != iclu; ++iclu ) {
    if ( ( *iclu ).second->seed().calo() != CaloCellCode::HcalCalo ) {
      ...
      this->updatePFCaloClusters( *theParticle, cellID ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
...
StatusCode ParticleFlow::treatNeutralRecovery( std::vector<LHCb::PFParticle*>& PFParticles ) {
  ...
  for ( std::map<unsigned int, LHCb::PFCaloCluster*>::const_iterator i_pfClu = m_PFCaloClusters.begin();
    ...
    verbose() << "   Main available cluster " << ( *i_pfClu ).second->seed().all() << " "
              << ( ( ( *i_pfClu ).second )->particleList() ).size() << " "
              << ( ( ( *i_pfClu ).second )->particleList() ).front() << endmsg;
    ...
    while ( !exit ) {
      ...
      while ( !iterated ) {
        ...
        if ( partLists_level[level][it]->info( CaloType, -100. ) > -1 &&
             ...
             m_PFCaloClusters[partLists_level[level][it]->info( CaloType, -100. )]->status() ==
                 LHCb::PFCaloCluster::AvailableForNeutralRecovery ) {
          // level up
    ...
    verbose() << "Super cluster size: " << SuperClusters.size()
              << " Number of particles associated to it: " << SuperParticles.size() << endmsg;
...
StatusCode ParticleFlow::createHiddenNeutral( const std::vector<LHCb::PFCaloCluster*>&    clusters,
                                              ...
                                              std::vector<LHCb::PFParticle*>&             PFParticles ) {
  // Loop over the particles. For each one get the most probable response as function of the associated cluster and its
  // direction Get the direction and Energy of each cluster Do the barycenter: di.Ei.Xi/di.Ei with di=1 for cluster and
  // di=-1 for particles
  ...
  for ( std::vector<LHCb::PFCaloCluster*>::const_iterator i_c = clusters.begin(); clusters.end() != i_c; ++i_c ) {
    ...
    if ( ( *i_c )->seed().calo() == CaloCellCode::HcalCalo )
      nSatCellHCAL += this->numberOfSaturatedCells( *i_c );
    else
      nSatCellECAL += this->numberOfSaturatedCells( *i_c );
  }
  ...
  if ( Barycenter[5] > m_MinE ) {
    double pt =
        abs( sin( atan( sqrt( Barycenter[0] * Barycenter[0] + Barycenter[1] * Barycenter[1] ) / Barycenter[2] ) ) *
             Barycenter[5] );
    if ( pt > m_MinEt ) {
...
Gaudi::Vector6 ParticleFlow::expectedEnergy( const LHCb::PFParticle* p ) {
  ...
  if ( theType == LHCb::PFParticle::Muon ) {
    ...
    if ( hasHCAL && hasECAL ) {
      if ( m_MC )
        expectedE = m_ECALElectron_MC->Eval( momentum ) * momentum;
      else
        expectedE = m_ECALElectron_Data->Eval( momentum ) * momentum;
    ...
    if ( hasHCAL && hasECAL ) {
      if ( m_MC )
        expectedE = m_ECALHCALHadron_MC->Eval( momentum ) * momentum;
      else
        expectedE = m_ECALHCALHadron_Data->Eval( momentum ) * momentum;
      // Put the parametrisation
    } else if ( hasHCAL && !hasECAL ) {
      if ( m_MC )
        expectedE = m_HCALHadron_MC->Eval( momentum ) * momentum;
      else
        expectedE = m_HCALHadron_Data->Eval( momentum ) * momentum;
      // Put the parametrisation
    } else if ( !hasHCAL && hasECAL ) {
      if ( m_MC )
        expectedE = m_ECALHadron_MC->Eval( momentum ) * momentum;
      else
        expectedE = m_ECALHadron_Data->Eval( momentum ) * momentum;
      // Put the parametrisation
    }
  }
  ...
  //}
    ...
    Warning( "Undefined case in neutral recovery " ).ignore(); // Might be a neutral particle....
  }
  ///// RECHECK this one....
...
StatusCode
ParticleFlow::checkClustersForNeutralRecovery( LHCb::PFParticle&                                           theParticle,
  ...
  if ( m_onlyBest ) {
    ...
    if ( ecalAssoc.size() > 0 ) {
      if ( ecalAssoc.front().weight() < m_Chi2ECALCut ) {
        ...
        m_PFCaloClusters[ecalAssoc.front().to()->seed().all()]->setStatus(
            LHCb::PFCaloCluster::AvailableForNeutralRecovery );
    ...
    if ( m_useHCAL ) {
      if ( hcalAssoc.size() > 0 ) {
        ...
        if ( ( hcalAssoc.front().weight() < m_Chi2HCAL0Cut && hcalAssoc.front().to()->e() <= m_Chi2HCAL0CutEValue ) ||
          ...
          m_PFCaloClusters[hcalAssoc.front().to()->seed().all()]->setStatus(
              LHCb::PFCaloCluster::AvailableForNeutralRecovery );
          // In the case of non composite particles (ie itr is theParticle)
          if ( theParticle.PFType() == LHCb::PFParticle::ChargedInfMomentum && m_doNotUseInfMominNR ) {
            m_PFCaloClusters[hcalAssoc.front().to()->seed().all()]->setStatus(
                LHCb::PFCaloCluster::ContainsInfMomentum );
          }
        }
      }
    }
