...
//=============================================================================
LHCb::PFParticle::PFParticle( const Gaudi::Vector6& barycenter, const double oldEnergy ) : Particle() {
...
//=============================================================================
LHCb::PFParticle::PFParticle(
    const LHCb::ProtoParticle* pp, int trackTag,
  ...
  if ( trackTag == TurnTo0Momentum ) {
    ...
    if ( this->particleID().abspid() == 11 )
      this->setPFType( PFParticle::Electron );
    else if ( this->particleID().abspid() == 13 )
      this->setPFType( PFParticle::Muon );
    else
      this->setPFType( PFParticle::ChargedHadron );
  }
  ...
  // TODO: Append the clusters and saturation values --> this might be done at the Neutral reco level??? because here we
  // do not have the saturation info anymore
}
