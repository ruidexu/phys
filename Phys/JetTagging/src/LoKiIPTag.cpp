...
DECLARE_COMPONENT( LoKi::IPTag )

// ============================================================================
// Standard Constructor
LoKi::IPTag::IPTag( const std::string& type, const std::string& name, const IInterface* parent )
    : GaudiTool( type, name, parent ), m_Jet( 0 ), m_JetVertex( 0 ), m_CleanedDaugthers( 0 ), m_distCalc( 0 ) {
  // Interface
  declareInterface<IJetTagTool>( this );

  // Algorithm related properties
  declareProperty( "AlgorithmMode", m_AlgMode = "SumTracks",
                   "Algorithm mode, either SumTracks (default) or FirstTrack or SecondTrack or ThirdTrack" );

  declareProperty( "JetDaugthersTrackCutChi", m_DauTrackCutChi = 0.01,
                   "JetDaugthers Selection on Minimum Chi Squared Probability" );

  declareProperty( "JetDaugthersTrackCutGhost", m_DauTrackCutGhost = 0.1,
...
StatusCode LoKi::IPTag::removeV0() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << inputDaus.size() << " daughters found in the jet" << endmsg;
  ...
  for ( LHCb::Particle::ConstVector::const_iterator iDau = inputDaus.begin(); iDau != inputDaus.end(); ++iDau ) {
    ...
    for ( LHCb::Particle::ConstVector::const_iterator iDau2 = iDau + 1; iDau2 != inputDaus.end(); ++iDau2 ) {
      ...
      if ( ( std::fabs( sum.M() - MK0 ) < m_DMK0 ) || ( std::fabs( sum.M() - MLambda ) < m_DMLambda ) ) {
        // tag particles K0
        if ( VOTag.count( protoparticle->key() ) < 0.5 ) VOTag[protoparticle->key()] = 1.;
        if ( VOTag.count( protoparticle2->key() ) < 0.5 ) VOTag[protoparticle2->key()] = 1.;
      }
    }

    // Remove identified Kshorts and Lambda
    if ( VOTag.count( protoparticle->key() ) > 0.5 ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "V0 daughter candidate" << endmsg;
      continue;
    }
    ...
    if ( chiSquared < m_DauTrackCutChi ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "Daugther failed chi selection " << chiSquared << endmsg;
    ...
    if ( ghost > m_DauTrackCutGhost ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "Daugther failed ghost selection " << ghost << endmsg;
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << m_CleanedDaugthers->size() << " good daugthers found" << endmsg;
...
bool LoKi::IPTag::calculateJetProperty( const LHCb::Particle* jet, std::map<std::string, double>& jetWeight ) {
  ...
  for ( LHCb::Particles::const_iterator iDau = m_CleanedDaugthers->begin(); iDau != m_CleanedDaugthers->end();
        ++iDau ) {
    const LHCb::Particle* jetdau = *iDau;

    double ipDist, ipSig;
    m_distCalc->distance( jetdau, m_Jet, ipDist, ipSig ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
    ipSig = std::sqrt( ipSig );

    Gaudi::XYZVector ipSeg;
    LHCb::VertexBase jetVertex = (LHCb::VertexBase)m_JetVertex;
    m_distCalc->distance( jetdau, &jetVertex, ipSeg ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
  ...
  if ( SecondSignValue > m_secCut && SumSignValue > m_sumCut ) tag_global = 1;
