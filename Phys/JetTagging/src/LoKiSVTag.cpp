...
DECLARE_COMPONENT( LoKi::SVTag )

// ============================================================================
// Standard Constructor
LoKi::SVTag::SVTag( const std::string& type, const std::string& name, const IInterface* parent )
    ...
    , m_seedFinderName( "LoKi__SeedFinderNTrks" ) // runs ntrack seeder by default
{
  ...
  declareProperty( "AlgorithmMode", m_AlgMode = "SigMax" );

  declareProperty( "SeedFinder", m_seedFinderName );
  return;
}
...
StatusCode LoKi::SVTag::setJetVertex() {
  ...
  if ( m_Jet->referencePoint() != Gaudi::XYZPoint( 0, 0, 0 ) ) {
    ...
    m_JetVertex.setCovMatrix( m_Jet->posCovMatrix() );
...
bool LoKi::SVTag::calculateJetProperty( const LHCb::Particle* jet, std::map<std::string, double>& jetWeight ) {
  ...
  m_seedFinder->makeJets( m_JetInputs, m_JetVertex, jetSeeds ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );

  double lengthMax = -999, lengthMaxN = -999, lengthMaxM = -999, lengthMaxChi2NDF = -999, lengthSum = 0,
         lengthMaxDirAngleJ2S = -999, lengthMaxDirAngleS2S = -999;

  double sigMax = -999, sigMaxN = -999, sigMaxM = -999, sigMaxChi2NDF = -999, sigSum = 0, sigMaxDirAngleJ2S = -999,
  ...
  if ( jetSeeds.size() > 0 ) {
    for ( unsigned iSeed = 0; iSeed < jetSeeds.size(); iSeed++ ) {
      ...
      m_distCalc
          ->distance( (LHCb::VertexBase*)m_JetVertex.clone(), (LHCb::VertexBase*)jetSeeds.at( iSeed )->endVertex(),
                      ipDist, ipChi )
      ...
      Gaudi::XYZVector jetRef =
          Gaudi::XYZVector( m_Jet->referencePoint().X(), m_Jet->referencePoint().Y(), m_Jet->referencePoint().Z() );
      Gaudi::XYZVector seedDecay = Gaudi::XYZVector( jetSeeds.at( iSeed )->endVertex()->position().X(),
                                                     jetSeeds.at( iSeed )->endVertex()->position().Y(),
                                                     jetSeeds.at( iSeed )->endVertex()->position().Z() );
