...
DECLARE_COMPONENT( LoKi::ElectronTag )

// ============================================================================
// Standard Constructor
LoKi::ElectronTag::ElectronTag( const std::string& type, const std::string& name, const IInterface* parent )
    : GaudiTool( type, name, parent ), m_Jet( 0 ), m_JetVertex( 0 ), m_Electrons( 0 ), m_distCalc( 0 ) {
  // Interface
  ...
  // Electron selections
  declareProperty( "ElectronCutDR", m_ElectronCutDR = 0.3,
                   "Electron Selection on Maximum Delta R between Jet and Electron" );

  declareProperty( "ElectronCutPT", m_ElectronCutPT = 400, "Electron Selection on Minimum PT" );

  declareProperty( "ElectronCutPTRel", m_ElectronCutPTRel = 400., "Electron Selection on Minimum PtRel" );

  declareProperty( "ElectronTrackCutChi", m_ElectronTrackCutChi = 0.01,
                   "Electron Selection on Minimum Chi Squared Probability" );

  declareProperty( "ElectronTrackCutIP", m_ElectronTrackCutIP = 9.,
                   "Electron Selection on Minimum Signed IP Significance" );

  declareProperty( "ElectronTrackCutGhost", m_ElectronTrackCutGhost = 0.1,
...
StatusCode LoKi::ElectronTag::getElectrons() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << inputElectrons.size() << " electron candidates found" << endmsg;
  ...
  for ( LHCb::Particle::ConstVector::const_iterator iElectron = inputElectrons.begin();
        iElectron != inputElectrons.end(); ++iElectron ) {
    ...
    if ( electronPID == 11 ) { isElectron = true; }

    if ( !isElectron ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "Electron candidate failed isElectron selection" << endmsg;
    ...
    if ( deltaR > m_ElectronCutDR ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "Electron candidate failed dR selection " << deltaR << endmsg;
    ...
    if ( chiSquared < m_ElectronTrackCutChi ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "Electron candidate failed chi selection " << chiSquared << endmsg;
    ...
    if ( ghost > m_ElectronTrackCutGhost ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "Electron candidate failed ghost selection " << ghost << endmsg;
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << m_Electrons->size() << " good electrons found" << endmsg;
...
bool LoKi::ElectronTag::calculateJetProperty( const LHCb::Particle* jet, std::map<std::string, double>& jetWeight ) {
  ...
  for ( LHCb::Particles::const_iterator iElectron = m_Electrons->begin(); iElectron != m_Electrons->end();
        ++iElectron ) {
    ...
    double ipDist, ipSig;
    m_distCalc->distance( electron, m_Jet, ipDist, ipSig ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
    ipSig = std::sqrt( ipSig );

    Gaudi::XYZVector ipSeg;
    LHCb::VertexBase jetVertex = (LHCb::VertexBase)m_JetVertex;
    m_distCalc->distance( electron, &jetVertex, ipSeg ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
    ...
    if ( ipSignSig > bestIPSig ) { bestIPSig = ipSignSig; }
  }
  ...
  if ( bestPtRel > m_ElectronCutPTRel ) tag_global = 1;
