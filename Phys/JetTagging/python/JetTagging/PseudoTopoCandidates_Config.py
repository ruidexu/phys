...
class PseudoTopoCandidatesConf(object):
    ...
    def filterSelection(self, name, code, inputs, preambulo=None):
        ...
        return Selection(
            name + self.name, Algorithm=alg, RequiredSelections=inputs)
    ...
    def topoInputs(self, tag, inputs):
        ...
        return self.filterSelection(tag + 'TopoInputs', self.topoInputsCuts(),
                                    inputs)
    ...
    def makeTopoAllNBody(self, n, inputs, sumptcut=-1):
        decay = [[
            ...
            "K*(892)0 -> KS0 KS0"
                 ...
                 [
                     "B0 -> Delta0 K-", "B0 -> Delta0 K+", "B0 -> Delta0 KS0",
        ...
        cp = CombineParticles(
            DecayDescriptors=decay[n - 2],
            CombinationCut=comboCuts,
            MotherCut=momCuts)
        return Selection(
            "TopoAll%dBodySel" % n, Algorithm=cp, RequiredSelections=inputs)
    ...
    def topo2for3(self, all2):
        return self.filterSelection('Topo2for3Sel', '(M < 10000*MeV)', [all2])
    ...
    def topoSubPID(self, name, inputs):
        ...
        sel = Selection(
            name + 'FinalSel', Algorithm=sub, RequiredSelections=[inputs])
        filter = "INTREE(ID=='B0')"
    ...
    def makeTopoCands(self, inputs, addsumpt):
        ...
        return MergedSelection(
            self.name, [topo2_bdt, topo3_bdt, topo4_bdt])  #[topo2,topo3,topo4]
