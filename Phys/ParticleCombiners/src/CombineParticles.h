 ...
 *                 by $Author:$
 */
class CombineParticles : public extends2<DaVinciAlgorithm, ISetInputParticles, IIncidentListener> {
public:
  // ==========================================================================
...
public:
   ...
   */
  CombineParticles( const std::string& name, // the algorithm instance name
...
protected:
   ...
   */
  bool treat_combination( const LHCb::Particle::ConstVector& combination ) const;
  // ==========================================================================
   ...
   */
  bool treat_mother( const LHCb::ParticleID& pid, const LHCb::Particle::ConstVector& combination,
   ...
   *  @return status code
   */
  StatusCode post_action( size_t nTotal, const bool problem, const LHCb::Particle::ConstVector& saved );
  // ==========================================================================
  /// monitor combinations ?
  bool monitorCombinations() const { return monitor() && !m_combinationMonitorCode.empty(); }
  /// monitor mother       ?
  bool monitorMother() const { return monitor() && !m_motherMonitorCode.empty(); }
  // ==========================================================================
  ...
  bool tooMuchCandidates( const size_t nGood, const Decays::Decay& decay ) const {
    if ( 0 < m_maxCand && m_maxCand <= nGood ) {
      Warning( "MaxCandidates exceeded for '" + decay.toString() + "'", StatusCode::FAILURE, 0 ).ignore();
  ...
  bool tooMuchCombinations( const double ncomb, const Decays::Decay& decay ) const {
    if ( 0 < m_maxComb && m_maxComb <= ncomb ) {
      Warning( "MaxCombinations exceeded for '" + decay.toString() + "'", StatusCode::FAILURE, 0 ).ignore();
...
private:
  ...
  StatusCode getHistoTool( IPlotTool*& histoTool, const std::string& name, const std::string& path );
  // ==========================================================================
public:
  // ==========================================================================
  /// the handle for the property update
  void propertyHandler1( Gaudi::Details::PropertyBase& p );
  void propertyHandler2( Gaudi::Details::PropertyBase& p );
  // ==========================================================================
public:
  // ==========================================================================
...
protected:
  // ==========================================================================
  SmartIF<IIncidentSvc> incSvc() const {
    if ( !m_incSvc ) { m_incSvc = service( "IncidentSvc", true ); }
    return m_incSvc;
  }
  // ==========================================================================
private:
