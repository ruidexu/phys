...
std::string CombineParticles::preambulo() const {
  std::string result;
  for ( std::vector<std::string>::const_iterator iline = m_preambulo.begin(); m_preambulo.end() != iline; ++iline ) {
 ...
 */
// ============================================================================
CombineParticles::CombineParticles( const std::string& name, // the algorithm instance name
  ...
  //
  declareProperty( "DecayDescriptors", m_decayDescriptors, "The list of (simple) decays to be recontructed/selected" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler1, this );
  {
    auto p = Gaudi::Utils::getProperty( this, "DecayDescriptor" );
    if ( p && !p->updateCallBack() ) { p->declareUpdateHandler( &CombineParticles::propertyHandler1, this ); }
  }
  //
  ...
  declareProperty( "Preambulo", m_preambulo, "The preambulo to be used for Bender/Python script" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler1, this );
  //
  declareProperty( "DaughtersCuts", m_daughterCuts, "The map of cuts, to be applied for daughter particles" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler1, this );
  //
  declareProperty( "CombinationCut", m_combinationCut, "The cut to be applied for combination of good daughters" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler1, this );
  //
  declareProperty( "MotherCut", m_motherCut,
      ...
      ->declareUpdateHandler( &CombineParticles::propertyHandler1, this );
  //
  declareProperty( "DaughtersPlots", m_daughtersPlotsName, "The type/name for 'Daughters   Plots' tool " )
      ->declareUpdateHandler( &CombineParticles::propertyHandler2, this );
  //
  declareProperty( "CombinationPlots", m_combinationPlotsName, "The type/name for 'Combination Plots' tool " )
      ->declareUpdateHandler( &CombineParticles::propertyHandler2, this );
  //
  declareProperty( "MotherPlots", m_motherPlotsName, "The type/name for 'Mother      Plots' tool " )
      ->declareUpdateHandler( &CombineParticles::propertyHandler2, this );
  //
  declareProperty( "DaughtersPlotsPath", m_daughtersPlotsPath, "The path for 'Daughters   Plots'" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler2, this );
  //
  declareProperty( "CombinationPlotsPath", m_combinationPlotsPath, "The path for 'Combination Plots'" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler2, this );
  //
  declareProperty( "MotherPlotsPath", m_motherPlotsPath, "The path for 'Mother      Plots'" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler2, this );
  //
  // The general flag to switch on/off the monitoring
  declareProperty( "Monitor", m_monitor, "The general flag to switch on/off the monitoring" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler1, this );
  //
  // The map of monitoring functors for daughter particles
  declareProperty( "DaughtersMonitors", m_daughterMonitors, "The map of monitoring functors for daughter particles" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler1, this );
  //
  // The monitoring functor for combinations
  declareProperty( "CombinationMonitor", m_combinationMonitorCode,
                   "The monitoring functor for combinations before the cuts" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler1, this );
  //
  // The monitoring functor for mother particles
  declareProperty( "MotherMonitor", m_motherMonitorCode, "The monitoring functor for mother particles after all cuts" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler1, this );
  // Histogramming
  declareProperty( "HistoProduce", m_produceHistos = false, "Switch on/off the production of histograms" )
      ->declareUpdateHandler( &CombineParticles::propertyHandler2, this );
  //
  declareProperty( "MaxCombinations", m_maxComb, "The maximal number of combinations (per channel) to be processed" );
  declareProperty( "StopAtMaxCombinations", m_maxCombStop,
                   "Stop at the maximal number of combinations (per channel) " );
  declareProperty( "MaxCandidates", m_maxCand, "The maximal number of candidates   (per channel) to be processed" );
  declareProperty( "StopAtMaxCandidates", m_maxCandStop, "Stop at the maximal number of candidates (per channel) " );
  declareProperty( "StopIncidentType", m_stopIncidentName,
...
StatusCode CombineParticles::decodeAllCuts() {
  ...
  for ( Map::const_iterator ic = m_daughterCuts.begin(); m_daughterCuts.end() != ic; ++ic ) {
    if ( ic->first.empty() ) { continue; }
    //
    { // check the correctness of particle name
      Assert( 0 != pid( ic->first ), " Invalid Particle Name is used as the key : '" + ( ic->first ) + "'" );
    }
    //
    MyCut      item;
    StatusCode sc = factory->get( ic->second, item.m_cut, preambulo() );
    if ( sc.isFailure() ) { return Error( "Unable to  decode the cut for '" + ic->first + "':" + ic->second, sc ); }
    ...
    if ( monitor() && !m_daughterMonitors.empty() ) {
      ...
      if ( m_daughterMonitors.end() != imoni && !imoni->second.empty() ) {
        ...
        item.m_moni = true;
        debug() << "The decoded monitor for '" + ( ic->first ) + "' is: " << item.m_moni << endmsg;
      }
    }
  ...
  // 2) decode the cut for the combination of good daughters
  {
    ...
    if ( sc.isFailure() ) { return Error( "Unable to  decode the 'A'-cut: " + m_combinationCut, sc ); }
  ...
  // 3) decode the final cut for the constructed mother particle
  {
    ...
    if ( sc.isFailure() ) { return Error( "Unable to  decode 'mother'-cut: " + m_motherCut, sc ); }
    debug() << "The decoded cut for 'mother' is: " << m_motherCut << endmsg;
  }
  // monitor combinations ?
  if ( monitor() && !m_combinationMonitorCode.empty() ) {
    StatusCode sc = factory->get( m_combinationMonitorCode, m_combinationMonitor, preambulo() );
    if ( sc.isFailure() ) {
      return Error( "Unable to  decode 'combination-monitor': " + m_combinationMonitorCode, sc );
    }
    debug() << "The decoded 'combination-monitor' is: " << m_combinationMonitor << endmsg;
  }
  // monitor mothers ?
  if ( monitor() && !m_motherMonitorCode.empty() ) {
    StatusCode sc = factory->get( m_motherMonitorCode, m_motherMonitor, preambulo() );
    if ( sc.isFailure() ) { return Error( "Unable to  decode 'mother-monitor': " + m_motherMonitorCode, sc ); }
    debug() << "The decoded 'mother-monitor' is: " << m_motherMonitor << endmsg;
  }
  //
  StatusCode sc = decodeRest( factory );
  if ( sc.isFailure() ) { return Error( "Unable to decode the rest", sc ); }
  //
 ...
 */
// ============================================================================
StatusCode CombineParticles::decodeRest( LoKi::IHybridFactory* /* factory */ ) { return StatusCode::SUCCESS; }
// ============================================================================
...
StatusCode CombineParticles::initialize() {
  ...
  if ( !m_vertexFitNames.empty() ) {
    warning() << "===============================================================================" << endmsg;
    warning() << " The property 'VertexFitters' has been set for an instance of CombineParticles" << endmsg;
  ...
  // setup sentry/guard
  Gaudi::Utils::AlgContext sentry( this, ctx );

  // the actual tuning/decoding
  sc = updateMajor();
  if ( sc.isFailure() ) { return Error( "Error from updateMajor", sc ); } // RETURN
...
StatusCode CombineParticles::pre_action() {
  ...
  if ( m_to_be_updated2 ) {
    ...
    if ( sc.isFailure() ) { return Error( "Error from updateHistos", sc ); } // RETURN
  }
  //
...
// ============================================================================
StatusCode CombineParticles::get_daughters( CombineParticles::Selected& daughters ) {
  //
  // get the particles from the local storage
  //
  const LHCb::Particle::ConstVector& particles = m_useInputParticles ? m_inputParticles : i_particles();
  //
  if ( m_useInputParticles && msgLevel( MSG::INFO ) ) {
    info() << " The external set of " << particles.size() << " particles is used instead of InputLocations " << endmsg;
  }
  //
  if ( msgLevel( MSG::VERBOSE ) ) {
    for ( LHCb::Particle::ConstVector::const_iterator i = particles.begin(); i != particles.end(); ++i ) {
      verbose() << "Input :  " << ( *i )->key() << " " << ( *i )->particleID().pid() << " " << ( *i )->momentum()
                << endmsg;
    }
  }
  //
  ...
  for ( MyCuts::const_iterator idau = m_cuts.begin(); m_cuts.end() != idau; ++idau ) {
    Selected::Range r = daughters.add( idau->m_name, particles.begin(), particles.end(),
    ...
    if ( msgLevel( MSG::VERBOSE ) ) {
      for ( Selected::Range::const_iterator i = r.begin(); i != r.end(); ++i )
        verbose() << "Select: " << ( *i )->key() << " " << ( *i )->particleID().pid() << " " << ( *i )->momentum()
                  << endmsg;
    }
    // make plots ?
    if ( m_daughtersPlots ) {
      StatusCode sc = m_daughtersPlots->fillPlots( r.begin(), r.end(), r.name() );
      if ( sc.isFailure() ) { Warning( "Error from DaughterPlots", sc ).ignore(); }
    }
    // monitor
    if ( monitor() && idau->m_moni ) { idau->m_monitor( LHCb::Particle::ConstVector( r.begin(), r.end() ) ); }
    //
  }
  //
  return StatusCode::SUCCESS;
 ...
 */
// =============================================================================
bool CombineParticles::treat_combination( const LHCb::Particle::ConstVector& combination ) const {
  //
  if ( msgLevel( MSG::VERBOSE ) ) {
    for ( LHCb::Particle::ConstVector::const_iterator i = combination.begin(); i != combination.end(); ++i ) {
      verbose() << "New Com: " << ( *i )->key() << " " << ( *i )->particleID().pid() << " " << ( *i )->momentum()
                << endmsg;
    }
  }
  //
 ...
 */
// =============================================================================
bool CombineParticles::treat_mother( const LHCb::ParticleID& pid, const LHCb::Particle::ConstVector& combination,
                                     const IParticleCombiner* combiner, LHCb::Particle::ConstVector& saved ) {
  ...
  if ( sc.isFailure() ) {
    ...
    Print( mess, sc, MSG::DEBUG ).ignore();
    ++counter( mess );
    return false; // RETURN
  }
  //
 ...
 */
// ============================================================================
StatusCode CombineParticles::post_action( size_t nTotal, const bool problem,
  ...
  if ( problem ) {
    if ( msgLevel( MSG::DEBUG ) ) { debug() << "A problem with combinatorics has occured" << endmsg; }
    if ( !m_stopIncidentName.empty() ) { incSvc()->fireIncident( Incident( name(), m_stopIncidentName ) ); }
  }
  //
  return StatusCode::SUCCESS;
}
...
StatusCode CombineParticles::execute() {
  ...
  if ( sc.isFailure() ) { return Error( "Error from pre-action conditions", sc ); }
  ...
  if ( sc.isFailure() ) { return Error( "Error from get-daughters method", sc ); }
  ...
  for ( std::vector<Decays::Decay>::const_iterator idecay = m_decays.begin(); m_decays.end() != idecay && !problem;
        ++idecay ) {
    ...
    for ( Decays::Decay::Items::const_iterator child = items.begin(); items.end() != child; ++child ) {
      loop.add( daughters( child->name() ) );
    }

    // check for max
    if ( msgLevel( MSG::DEBUG ) || 0 < m_maxComb ) { counter( "# max size" + idecay->toString() ) += loop.size(); }
    //
    ...
    for ( ; loop.valid(); loop.next() ) // we are in the loop!
    {
      ...
      const bool pass_mother = treat_mother( idecay->mother().pid(), combination, combiner, saved );
      //
      cnt_m += pass_mother;
 ...
 */
// ============================================================================
StatusCode CombineParticles::setInput( const LHCb::Particle::ConstVector& input ) {
  m_inputParticles    = input;
  m_useInputParticles = true;
  info() << "The external set of " << input.size() << " particles will be used as input instead of InputLocations "
         << endmsg;
  return StatusCode::SUCCESS;
}
// =============================================================================
...
// =============================================================================
void CombineParticles::propertyHandler1( Gaudi::Details::PropertyBase& p ) {
  // no action if not initialized yet:
  if ( Gaudi::StateMachine::INITIALIZED > FSMState() ) { return; }
  /// mark as "to-be-updated"
  m_to_be_updated1 = true;
  Warning( "The structural property '" + p.name() + "' is updated. It will take affect at the next 'execute'",
           StatusCode::SUCCESS )
      .ignore();
...
// the handle for the property update
// =============================================================================
void CombineParticles::propertyHandler2( Gaudi::Details::PropertyBase& p ) {
  // no action if not initialized yet:
  if ( Gaudi::StateMachine::INITIALIZED > FSMState() ) { return; }
  /// mark as "to-be-updated"
  m_to_be_updated2 = true;
...
StatusCode CombineParticles::updateMajor() {
  ...
  {
    if ( m_decayDescriptors.empty() ) { m_decayDescriptors.push_back( getDecayDescriptor() ); }
    // locate the decoder tool:
    IDecodeSimpleDecayString* decoder = tool<IDecodeSimpleDecayString>( "DecodeSimpleDecayString:PUBLIC", this );
    //
    m_decays = Decays::decays( m_decayDescriptors, decoder );
    if ( m_decays.empty() ) { return Error( "Unable to decode DecayDescriptor(s) " ); } // RETURN
    // release the tool (not needed anymore)
    release( decoder ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
  }
  // 2) check the list of daughters, involved into all decays:
  {
    typedef std::set<std::string> PIDs;
    PIDs                          pids;
    for ( std::vector<Decays::Decay>::const_iterator idecay = m_decays.begin(); m_decays.end() != idecay; ++idecay ) {
      const Decays::Decay::Items& ds = idecay->daughters();
      for ( Decays::Decay::Items::const_iterator ic = ds.begin(); ds.end() != ic; ++ic ) { pids.insert( ic->name() ); }
    }
    ...
    for ( PIDs::const_iterator ipid = pids.begin(); pids.end() != ipid; ++ipid ) {
      ...
      { // play with daughter cuts
        ...
        if ( m_daughterCuts.end() != i1 && m_daughterCuts.end() != i2 ) {
        }                                      // nothing to do
        else if ( m_daughterCuts.end() != i1 ) // the cut for particle is already specified
        {
          m_daughterCuts[anti] = m_daughterCuts[*ipid];
        }                                      // use the same cust for antiparticle
        else if ( m_daughterCuts.end() != i2 ) // the cut for anti-particle is already specified
        {
          m_daughterCuts[*ipid] = m_daughterCuts[anti];
        }    // use the same cuts as particle
      ...
      if ( monitor() && !m_daughterMonitors.empty() ) {
        ...
        if ( m_daughterMonitors.end() != j1 && m_daughterMonitors.end() != j2 ) {
        } // nothing to do
        // the monitor for particle is already specified
        else if ( m_daughterMonitors.end() != j1 ) {
          m_daughterMonitors[anti] = m_daughterMonitors[*ipid];
        } // use the same monitor for antiparticle
        // the monitor for anti-particle is already specified
        else if ( m_daughterMonitors.end() != j2 ) {
          m_daughterMonitors[*ipid] = m_daughterMonitors[anti];
        } // use the same monitor as particle
      }
    }

    if ( msgLevel( MSG::DEBUG ) ) {
      debug() << "The updated map of daughter cuts is : " << Gaudi::Utils::toString( m_daughterCuts ) << endmsg;
    }

    if ( msgLevel( MSG::DEBUG ) && monitor() && !m_daughterMonitors.empty() ) {
      debug() << "The updated map of daughter monitors is : " << Gaudi::Utils::toString( m_daughterMonitors ) << endmsg;
    }
  }
  // 3) decode the cuts for the daughters:
  {
    StatusCode sc = decodeAllCuts();
    if ( sc.isFailure() ) { return Error( "The error from decodeAlLCuts", sc ); } // RETURN
  }
  //
...
// ============================================================================
StatusCode CombineParticles::getHistoTool( IPlotTool*& histoTool, const std::string& name, const std::string& path ) {
  if ( validPlots( name ) ) {
    histoTool = tool<IPlotTool>( name, this );
    if ( !path.empty() ) {
      StatusCode sc = m_daughtersPlots->setPath( path );
      if ( sc.isFailure() ) { return Error( "Unable to set Plots Path " + path + " for tool " + name, sc ); }
    }
  }
  return StatusCode::SUCCESS;
}
...
StatusCode CombineParticles::updateHistos() {
  // ==========================================================================
  if ( m_daughtersPlots ) {
    releaseTool( m_daughtersPlots ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
    m_daughtersPlots = 0;
  }
  if ( m_combinationPlots ) {
    releaseTool( m_combinationPlots ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
    m_combinationPlots = 0;
  }
  if ( m_motherPlots ) {
    releaseTool( m_motherPlots ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
    m_motherPlots = 0;
  }
  // ==========================================================================
  if ( m_produceHistos ) {
...
DECLARE_COMPONENT( CombineParticles )
// ============================================================================
// The END
// ============================================================================
