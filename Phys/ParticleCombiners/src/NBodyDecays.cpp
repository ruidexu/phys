...
namespace DaVinci {
  ...
  public:
     ...
     */
    N3BodyDecays( const std::string& name, // the algorithm instance name
                  ISvcLocator*       pSvc );     // the service locator
    // ========================================================================
  protected:
    // ========================================================================
    /// the handle for the property update
    void propertyHandler3( Gaudi::Details::PropertyBase& p ) { propertyHandler1( p ); }
  ...
  public:
     ...
     *  @param pSvc the service locator
     */
    N4BodyDecays( const std::string& name, // the algorithm instance name
                  ISvcLocator*       pSvc );     // the service locator
    // ========================================================================
  protected:
    // ========================================================================
    /// the handle for the property update
    void propertyHandler4( Gaudi::Details::PropertyBase& p ) { propertyHandler3( p ); }
    /// decode the specific information
    StatusCode decodeRest( LoKi::IHybridFactory* factory ) override;
  ...
  public:
     ...
     *  @param pSvc the service locator
     */
    N5BodyDecays( const std::string& name, // the algorithm instance name
                  ISvcLocator*       pSvc );     // the service locator
    // ========================================================================
  protected:
    // ========================================================================
    /// the handle for the property update
    void propertyHandler5( Gaudi::Details::PropertyBase& p ) { propertyHandler4( p ); }
    /// decode the specific information
    StatusCode decodeRest( LoKi::IHybridFactory* factory ) override;
  ...
  public:
     ...
     *  @param pSvc the service locator
     */
    N6BodyDecays( const std::string& name, // the algorithm instance name
                  ISvcLocator*       pSvc );     // the service locator
    // ========================================================================
  protected:
    // ========================================================================
    /// the handle for the property update
    void propertyHandler6( Gaudi::Details::PropertyBase& p ) { propertyHandler5( p ); }
    /// decode the specific information
    StatusCode decodeRest( LoKi::IHybridFactory* factory ) override;
  ...
  public:
     ...
     *  @param pSvc the service locator
     */
    N7BodyDecays( const std::string& name, // the algorithm instance name
                  ISvcLocator*       pSvc );     // the service locator
    // ========================================================================
  protected:
    // ========================================================================
    /// the handle for the property update
    void propertyHandler7( Gaudi::Details::PropertyBase& p ) { propertyHandler6( p ); }
    /// decode the specific information
    StatusCode decodeRest( LoKi::IHybridFactory* factory ) override;
  ...
  public:
     ...
     *  @param pSvc the service locator
     */
    N8BodyDecays( const std::string& name, // the algorithm instance name
                  ISvcLocator*       pSvc );     // the service locator
    // ========================================================================
  protected:
    // ========================================================================
    /// the handle for the property update
    void propertyHandler8( Gaudi::Details::PropertyBase& p ) { propertyHandler7( p ); }
    /// decode the specific information
    StatusCode decodeRest( LoKi::IHybridFactory* factory ) override;
...
namespace {
  ...
  bool _check_decays_( const std::vector<Decays::Decay>& decays, const unsigned short N ) {
    //
    for ( std::vector<Decays::Decay>::const_iterator idecay = decays.begin(); decays.end() != idecay; ++idecay ) {
 ...
 */
// ============================================================================
DaVinci::N3BodyDecays::N3BodyDecays( const std::string& name, // the algorithm instance name
                                     ISvcLocator*       pSvc )      // the service locator
  ...
  //
  declareProperty( "Combination12Cut", m_combination12Cut,
...
StatusCode DaVinci::N3BodyDecays::initialize() // standard initialization
{
  ...
  //
  if ( !_check_decays_( m_decays, 3 ) ) { return Error( "Invalid decay sructure is specified!" ); }
  //
...
StatusCode DaVinci::N3BodyDecays::decodeRest( LoKi::IHybridFactory* factory ) {
  ...
  if ( sc.isFailure() ) { return Error( "Unable to  decode 'Combination12Cut': " + m_combination12Cut, sc ); }
  debug() << "The decoded 'Combination12Cut' is: " << m_acut12 << endmsg;
  //
 ...
 */
// ============================================================================
DaVinci::N4BodyDecays::N4BodyDecays( const std::string& name, // the algorithm instance name
                                     ISvcLocator*       pSvc )      // the service locator
  ...
  //
  declareProperty( "Combination123Cut", m_combination123Cut,
...
StatusCode DaVinci::N4BodyDecays::initialize() // standard initialization
{
  ...
  if ( sc.isFailure() ) { return sc; }
  //
  if ( !_check_decays_( m_decays, 4 ) ) { return Error( "Invalid decay sructure is specified!" ); }
  //
  return StatusCode::SUCCESS;
}
// ============================================================================
...
StatusCode DaVinci::N4BodyDecays::decodeRest( LoKi::IHybridFactory* factory ) {
  ...
  if ( sc.isFailure() ) { return Error( "Unable to  decode 'Combination123Cut': " + m_combination123Cut, sc ); }
  debug() << "The decoded 'Combination123Cut' is: " << m_acut123 << endmsg;
  //
 ...
 */
// ============================================================================
DaVinci::N5BodyDecays::N5BodyDecays( const std::string& name, // the algorithm instance name
                                     ISvcLocator*       pSvc )      // the service locator
  ...
  //
  declareProperty( "Combination1234Cut", m_combination1234Cut,
...
StatusCode DaVinci::N5BodyDecays::initialize() // standard initialization
{
  ...
  if ( sc.isFailure() ) { return sc; }
  //
  if ( !_check_decays_( m_decays, 5 ) ) { return Error( "Invalid decay sructure is specified!" ); }
  //
  return StatusCode::SUCCESS;
}
// ============================================================================
...
StatusCode DaVinci::N5BodyDecays::decodeRest( LoKi::IHybridFactory* factory ) {
  ...
  if ( 0 == factory ) { return Error( "Invalid factory!" ); }
  //
  StatusCode sc = factory->get( m_combination1234Cut, m_acut1234, preambulo() );
  if ( sc.isFailure() ) { return Error( "Unable to  decode 'Combination1234Cut': " + m_combination1234Cut, sc ); }
  debug() << "The decoded 'Combination1234Cut' is: " << m_acut1234 << endmsg;
  //
 ...
 */
// ============================================================================
DaVinci::N6BodyDecays::N6BodyDecays( const std::string& name, // the algorithm instance name
                                     ISvcLocator*       pSvc )      // the service locator
  ...
  //
  declareProperty( "Combination12345Cut", m_combination12345Cut,
...
StatusCode DaVinci::N6BodyDecays::initialize() // standard initialization
{
  ...
  if ( sc.isFailure() ) { return sc; }
  //
  if ( !_check_decays_( m_decays, 6 ) ) { return Error( "Invalid decay sructure is specified!" ); }
  //
  return StatusCode::SUCCESS;
}
// ============================================================================
...
StatusCode DaVinci::N6BodyDecays::decodeRest( LoKi::IHybridFactory* factory ) {
  ...
  if ( 0 == factory ) { return Error( "Invalid factory!" ); }
  //
  StatusCode sc = factory->get( m_combination12345Cut, m_acut12345, preambulo() );
  if ( sc.isFailure() ) { return Error( "Unable to  decode 'Combination12345Cut': " + m_combination12345Cut, sc ); }
  debug() << "The decoded 'Combination12345Cut' is: " << m_acut12345 << endmsg;
  //
 ...
 */
// ============================================================================
DaVinci::N7BodyDecays::N7BodyDecays( const std::string& name, // the algorithm instance name
                                     ISvcLocator*       pSvc )      // the service locator
  ...
  //
  declareProperty( "Combination123456Cut", m_combination123456Cut,
...
StatusCode DaVinci::N7BodyDecays::initialize() // standard initialization
{
  ...
  if ( sc.isFailure() ) { return sc; }
  //
  if ( !_check_decays_( m_decays, 7 ) ) { return Error( "Invalid decay sructure is specified!" ); }
  //
  return StatusCode::SUCCESS;
}
// ============================================================================
...
StatusCode DaVinci::N7BodyDecays::decodeRest( LoKi::IHybridFactory* factory ) {
  ...
  if ( 0 == factory ) { return Error( "Invalid factory!" ); }
  //
  StatusCode sc = factory->get( m_combination123456Cut, m_acut123456, preambulo() );
  if ( sc.isFailure() ) { return Error( "Unable to  decode 'Combination123456Cut': " + m_combination123456Cut, sc ); }
  debug() << "The decoded 'Combination123456Cut' is: " << m_acut123456 << endmsg;
  //
 ...
 */
// ============================================================================
DaVinci::N8BodyDecays::N8BodyDecays( const std::string& name, // the algorithm instance name
                                     ISvcLocator*       pSvc )      // the service locator
  ...
  //
  declareProperty( "Combination1234567Cut", m_combination1234567Cut,
...
StatusCode DaVinci::N8BodyDecays::initialize() // standard initialization
{
  ...
  if ( sc.isFailure() ) { return sc; }
  //
  if ( !_check_decays_( m_decays, 8 ) ) { return Error( "Invalid decay sructure is specified!" ); }
  //
  return StatusCode::SUCCESS;
}
// ============================================================================
...
StatusCode DaVinci::N8BodyDecays::decodeRest( LoKi::IHybridFactory* factory ) {
  ...
  if ( 0 == factory ) { return Error( "Invalid factory!" ); }
  //
  StatusCode sc = factory->get( m_combination1234567Cut, m_acut1234567, preambulo() );
  if ( sc.isFailure() ) { return Error( "Unable to  decode 'Combination1234567Cut': " + m_combination1234567Cut, sc ); }
  debug() << "The decoded 'Combination1234567Cut' is: " << m_acut1234567 << endmsg;
  //
...
StatusCode DaVinci::N3BodyDecays::execute() {
  ...
  for ( std::vector<Decays::Decay>::const_iterator idecay = m_decays.begin(); m_decays.end() != idecay && !problem;
    ...
    //
    for ( Selected::Range::const_iterator it1 = child1.begin(); child1.end() != it1 && !problem; ++it1 ) {
      //
      ...
      for ( Selected::Range::const_iterator it2 = child2.begin(); child2.end() != it2 && !problem; ++it2 ) {
        //
        problem = problem || tooMuchCandidates( nGood, *idecay );
        if ( problem ) { break; } // BREAK the loop
        //
        ...
        for ( Selected::Range::const_iterator it3 = child3.begin(); child3.end() != it3 && !problem; ++it3 ) {
          //
          problem = problem || tooMuchCandidates( nGood, *idecay );
          if ( problem ) { break; } // BREAK the loop
          //
    ...
    for ( Selected::Range::const_iterator it1 = child1.begin(); child1.end() != it1 && !problem; ++it1 ) {
      ...
      for ( Selected::Range::const_iterator it2 = child2.begin(); child2.end() != it2 && !problem; ++it2 ) {
        ...
        for ( Selected::Range::const_iterator it3 = child3.begin(); child3.end() != it3 && !problem; ++it3 ) {
          ...
          for ( Selected::Range::const_iterator it4 = child4.begin(); child4.end() != it4 && !problem; ++it4 ) {
            //
            problem = problem || tooMuchCandidates( nGood, *idecay );
            if ( problem ) { break; } // BREAK the loop
            //
      ...
      for ( Selected::Range::const_iterator it2 = child2.begin(); child2.end() != it2 && !problem; ++it2 ) {
        ...
        for ( Selected::Range::const_iterator it3 = child3.begin(); child3.end() != it3 && !problem; ++it3 ) {
          ...
          for ( Selected::Range::const_iterator it4 = child4.begin(); child4.end() != it4 && !problem; ++it4 ) {
            ...
            for ( Selected::Range::const_iterator it5 = child5.begin(); child5.end() != it5 && !problem; ++it5 ) {
        ...
        for ( Selected::Range::const_iterator it3 = child3.begin(); child3.end() != it3 && !problem; ++it3 ) {
          ...
          for ( Selected::Range::const_iterator it4 = child4.begin(); child4.end() != it4 && !problem; ++it4 ) {
            ...
            for ( Selected::Range::const_iterator it5 = child5.begin(); child5.end() != it5 && !problem; ++it5 ) {
              ...
              for ( Selected::Range::const_iterator it6 = child6.begin(); child6.end() != it6 && !problem; ++it6 ) {
                ...
                //
                // good & unique ?
                if ( !compare( c1, c2, c3, c4, c5, c6 ) ) { continue; } // CONTINUE, next c6
                //
                combination[5] = c6;
          ...
          for ( Selected::Range::const_iterator it4 = child4.begin(); child4.end() != it4 && !problem; ++it4 ) {
            ...
            for ( Selected::Range::const_iterator it5 = child5.begin(); child5.end() != it5 && !problem; ++it5 ) {
              ...
              for ( Selected::Range::const_iterator it6 = child6.begin(); child6.end() != it6 && !problem; ++it6 ) {
                ...
                for ( Selected::Range::const_iterator it7 = child7.begin(); child7.end() != it7 && !problem; ++it7 ) {
                  ...
                  //
                  // good & unique ?
                  if ( !compare( c1, c2, c3, c4, c5, c6, c7 ) ) { continue; } // CONTINUE, next c6
                  //
                  combination[6] = c7;
            ...
            for ( Selected::Range::const_iterator it5 = child5.begin(); child5.end() != it5 && !problem; ++it5 ) {
              ...
              for ( Selected::Range::const_iterator it6 = child6.begin(); child6.end() != it6 && !problem; ++it6 ) {
                ...
                for ( Selected::Range::const_iterator it7 = child7.begin(); child7.end() != it7 && !problem; ++it7 ) {
                  ...
                  for ( Selected::Range::const_iterator it8 = child8.begin(); child8.end() != it8 && !problem; ++it8 ) {
                    ...
                    //
                    // good & unique ?
                    if ( !compare( c1, c2, c3, c4, c5, c6, c7, c8 ) ) { continue; } // CONTINUE, next c6
                    //
                    combination[7] = c8;
...
// ============================================================================
DECLARE_COMPONENT( DaVinci::N3BodyDecays )
DECLARE_COMPONENT( DaVinci::N4BodyDecays )
DECLARE_COMPONENT( DaVinci::N5BodyDecays )
DECLARE_COMPONENT( DaVinci::N6BodyDecays )
DECLARE_COMPONENT( DaVinci::N7BodyDecays )
DECLARE_COMPONENT( DaVinci::N8BodyDecays )
