...
struct GAUDI_API ITisTos : extend_interfaces<IAlgTool> {
  ...
  public:
    ...
    void set_decision( bool dec = true ) {
      if ( dec ) {
        m_tistostob |= kDecision;
      } else {
        m_tistostob &= ( ~kDecision );
      };
    }
  ...
  /// set minimal fraction of trigger hits to be found on signal for TOS - hit types implementation dependent (<=0 don't
  /// use)
  virtual bool setTOSFrac( unsigned int hitType, double tosFrac ) = 0;

  /// set maximal fraction of trigger hits to be found on signal for TIS - (should be below TOSFrac except when
  /// TOSFrac<=0)
  virtual bool setTISFrac( unsigned int hitType, double tisFrac ) = 0;

  /// set minimal fractions of trigger hits to be found on signal for TOS - (<=0 don't use this type neither in TOS nor
  /// TIS)
  virtual bool setTOSFrac( std::vector<double> tosFrac ) = 0;

  /// set maximal fractions of trigger hits to be found on signal for TIS - (should be below TOSFrac except when
  /// TOSFrac<=0)
