...
struct GAUDI_API ITriggerSelectionTisTos : extend_interfaces<IParticleTisTos> {
  ...
  /** @class TisTosDecision ITriggerSelectionTisTos.h
   *
   *
   *  @author Tomasz Skwarnicki
   *  @date   2007-08-06
  ...
  /// list of HltObjectSummaries from Selection Summary satisfying TOS,TIS requirements (define Offline Input before
  /// calling)
  virtual std::vector<const LHCb::HltObjectSummary*>
  hltSelectionObjectSummaries( const std::string& selectionName, unsigned int tisRequirement = kAnything,
                               unsigned int tosRequirement = kAnything, unsigned int tpsRequirement = kAnything ) = 0;
  ...
  void selectionTisTos( const std::string& selectionName, bool& decision, bool& tis, bool& tos ) {
    const unsigned int result = tisTosSelection( selectionName );
    decision                  = ( ( result & kDecision ) != 0 );
    tis                       = ( ( result & kTIS ) != 0 );
    tos                       = ( ( result & kTOS ) != 0 );
  }

  /// multiple Trigger Selection Summaries TisTos (set selectionOR=false for an AND) (define Offline Input before
  /// calling)
  void selectionTisTos( const std::vector<std::string>& selectionNames, bool& decision, bool& tis, bool& tos,
  ...
  TisTosDecision selectionTisTos( const std::string& selectionName ) {
    ...
    selectionTisTos( selectionName, decision, tis, tos );
  ...
  TisTosDecision selectionTisTos( const std::vector<std::string>& selectionNames,
    ...
    bool decision( false ), tis( false ), tos( false );
    selectionTisTos( selectionNames, decision, tis, tos, selectionOR );
    return TisTosDecision( decision, tis, tos );
  }
  ...
  void selectionTisTos( const LHCb::Particle& particle, const std::string& selectionName, bool& decision, bool& tis,
                        bool& tos ) {
  ...
  /// shortcut to define Offline Input and get Tis,Tos results in one call
  void selectionTisTos( const LHCb::ProtoParticle& protoParticle, const std::string& selectionName, bool& decision,
                        bool& tis, bool& tos ) {
  ...
  /// shortcut to define Offline Input and get Tis,Tos results in one call
  void selectionTisTos( const LHCb::Track& track, const std::string& selectionName, bool& decision, bool& tis,
                        bool& tos ) {
  ...
  /// shortcut to define Offline Input and get Tis,Tos results in one call
  void selectionTisTos( const std::vector<LHCb::LHCbID>& hitlist, const std::string& selectionName, bool& decision,
                        bool& tis, bool& tos ) {
  ...
  // shortcuts to define Offline Input and get matched TOS objects in one call
  // std::vector<const LHCb::RecVertex*> matchedTOSVertices(const LHCb::Particle & particle, const std::string &
  // selectionName )
  //  { setOfflineInput(particle); return  matchedTOSVertices(selectionName); }

  // shortcuts to define Offline Input and get matched TOS objects in one call
  // std::vector<const LHCb::Particle*>  matchedTOSParticles(const LHCb::Particle & particle, const std::string &
  // selectionName )
