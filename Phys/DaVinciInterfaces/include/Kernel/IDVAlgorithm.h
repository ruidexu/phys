...
struct GAUDI_API IDVAlgorithm : extend_interfaces<INamedInterface> {
  ...
  virtual const ILifetimeFitter* lifetimeFitter( const std::string& nickname = "" ) const = 0;
  /// get the vertex fitter tool
  virtual const IVertexFit* vertexFitter( const std::string& nickname = "" ) const = 0;
  /// get particle re-fitter tool
  virtual const IParticleReFitter* particleReFitter( const std::string& nickname = "" ) const = 0;
  /// get particle filter tool
  virtual const IParticleFilter* particleFilter( const std::string& nickname = "" ) const = 0;
  /// Accessor for decay-tree fitter
  virtual IDecayTreeFit* decayTreeFitter( const std::string& nickname = "" ) const = 0;
  /// Accessor for ParticleCombiner tool
  virtual const IParticleCombiner* particleCombiner( const std::string& nickname = "" ) const = 0;
  /// Accessor for mass-fitter tool
  virtual const IMassFit* massFitter( const std::string& nickname = "" ) const = 0;
  /// Accessor for direction-fitter tool
  virtual const IDirectionFit* directionFitter( const std::string& nickname = "" ) const = 0;
  /// Accessor for primary vertex re-fitter tool
  virtual const IPVReFitter* primaryVertexReFitter( const std::string& nickname = "" ) const = 0;
