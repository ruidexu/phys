 ...
 * @par
 *   This tool extends functionality of @c ITriggerSelectionTisTos (from which it inherits). While @c
 * ITriggerSelectionTisTos member functions accept for trigger input only specific Trigger Selection names (which cannot
 * be checked for validity),
 *   @c ITriggerTisTos tool knows valid Trigger Selection names and their hierarchy. Therefore, trigger input can be
 * specified via trigger name pattern which contains wild characters. This allows user to define trigger structures
 * independently of the particular trigger configuration. Auxiliary output functions allow discovering valid Trigger
 * Selection names, as well as selections matching decision/TIS/TOS requirements.
 ...
 *   be passed as Offline Input. Passing ProtoParticle is not the same as passing the track the ProtoParticle may be
 * built on, since the tool can take input from PID info (e.g. to grab Muon hits for matching).
 * @par
 ...
 * @par
 *   Once the Offline Input is defined, many calls to the output functions can be made without re-entering the Offline
 * Input.
 * @par
 ...
 *   pre-processed and stored in an internal representation).
 * @par
 *   The tool will print a warning message if the passed Trigger Input pattern does not correspond to any trigger
 * selection.
 * @par
 *   User can obtain names of trigger selections corresponding to the Trigger Input by calling @c
 * triggerSelectionNames() with no arguments (input selection tree can be explored by calling @c
 * inputTriggerSelectionNames(selectionName) ).
 ...
 * @par
 *   The tool can also return HltObjectSummaries for trigger objects (Tracks, RecVertices, Particles etc.) responsible
 * for positive decisions via @c hltObjectSummaries() method, which takes optional arguments to restrict output to
 * specific TOS or TIS requirements.
 ...
 *   to the Trigger Input is satisfied with trigger tracks/vertices/particles which are not matched to the Offline
 ...
 *   tracks/vertices/particles which are matched to the Offline Input. Matching criteria are implementation dependent
 * and not necessarily the same as for TIS (i.e. not-TIS object is not necessarily the same as TOS object)
 * @par
...
struct GAUDI_API ITriggerTisTos : extend_interfaces<ITriggerSelectionTisTos> {
   ...
   *       Use ITriggerTisTos::kFalseRequired, ITriggerTisTos::kTrueRequired, ITriggerTisTos::kAnything to avoid magic
   * numbers.
   *  @par
   *       Some combinations make no sense, thus will always return empty vector (e.g. decision==false && tos==true).
   *  @par
   *       When no pattern is imposed (default) all Trigger Selection names corresponding to the Trigger Input are
   * returned.
   *  @par
   ...
   * numbers.
   *  @par
   *       When no pattern is imposed (default) HltObjectSummaries for all trigger objects in the Trigger Input are
   * returned.
   *  @par
  ...
  /// calculate decision,Tis,Tos  (for previously defined Trigger and Offline Inputs) - see inheritance for Offline
  /// Input def
  ...
  /// shortcut to define Trigger Input via name pattern and return decision,tis,tos with respect to previously set
  /// Offline Input
  void triggerTisTos( const std::string& selectionNameWithWildChar, bool& decision, bool& tis, bool& tos ) {
  ...
  /// shortcut to define Trigger Input via name patterns and return decision,tis,tos with respect to previously set
  /// Offline Input
  void triggerTisTos( const std::vector<std::string>& selectionNamesWithWildChar, bool& decision, bool& tis,
                      bool& tos ) {
  ...
  void triggerTisTos( const LHCb::Particle& particle, bool& decision, bool& tis, bool& tos ) {
    setOfflineInput( particle );
    triggerTisTos( decision, tis, tos );
  }

  /// shortcut to define Offline Input and calculate decision,tis,tos for previously defined Trigger Input
  void triggerTisTos( const LHCb::ProtoParticle& protoParticle, bool& decision, bool& tis, bool& tos ) {
    setOfflineInput( protoParticle );
    triggerTisTos( decision, tis, tos );
  }

  /// shortcut to define Offline Input and calculate decision,tis,tos for previously defined Trigger Input
  void triggerTisTos( const LHCb::Track& track, bool& decision, bool& tis, bool& tos ) {
    setOfflineInput( track );
    triggerTisTos( decision, tis, tos );
  }

  /// shortcut to define Offline Input and calculate decision,tis,tos for previously defined Trigger Input
  void triggerTisTos( const std::vector<LHCb::LHCbID>& hitlist, bool& decision, bool& tis, bool& tos ) {
  ...
  void triggerTisTos( const LHCb::Particle& particle, const std::string& selectionNameWithWildChar, bool& decision,
                      bool& tis, bool& tos ) {
    setOfflineInput( particle );
    setTriggerInput( selectionNameWithWildChar );
    triggerTisTos( decision, tis, tos );
  ...
  /// python friendly - shortcut to define Offline Input and Trigger Input via trigger name pattern and calculate
  /// decision,tis,tos
  TisTosDecision triggerTisTos( const LHCb::Particle& particle, const std::string& selectionNameWithWildChar ) {
    bool decision( false ), tis( false ), tos( false );
  ...
  /// shortcut to define Offline Input and Trigger Input via trigger name pattern and calculate decision,tis,tos
  void triggerTisTos( const LHCb::ProtoParticle& protoParticle, const std::string& selectionNameWithWildChar,
  ...
  /// shortcut to define Offline Input and Trigger Input via trigger name pattern and calculate decision,tis,tos
  void triggerTisTos( const LHCb::Track& track, const std::string& selectionNameWithWildChar, bool& decision, bool& tis,
                      bool& tos ) {
    setOfflineInput( track );
    setTriggerInput( selectionNameWithWildChar );
    triggerTisTos( decision, tis, tos );
  ...
  /// shortcut to define Offline Input and Trigger Input via trigger name pattern and calculate decision,tis,tos
  void triggerTisTos( const std::vector<LHCb::LHCbID>& hitlist, const std::string& selectionNameWithWildChar,
                      bool& decision, bool& tis, bool& tos ) {
    setOfflineInput( hitlist );
    setTriggerInput( selectionNameWithWildChar );
    triggerTisTos( decision, tis, tos );
  ...
  /// shortcut to set Trigger Input and return Trigger Sel. names matching dec.,tis,tos pattern for previously set
  /// Offline Input
  ...
  triggerSelectionNames( const LHCb::Particle& particle, const std::string& selectionNameWithWildChar,
                         unsigned int decisionRequirement = kAnything, unsigned int tisRequirement = kAnything,
                         unsigned int tosRequirement = kAnything, unsigned int tpsRequirement = kAnything ) {
    setOfflineInput( particle );
    setTriggerInput( selectionNameWithWildChar );
    return triggerSelectionNames( decisionRequirement, tisRequirement, tosRequirement, tpsRequirement );
  }
