...
// find seeds
// ===========================================================================
StatusCode LoKi::VVSeedFinder::makeJets( const IJetMaker::Input& input_, IJetMaker::Jets& jets_ ) const {
...
StatusCode LoKi::VVSeedFinder::makeJets( const IJetMaker::Input& input_, const LHCb::RecVertex& RecVert_,
  ...
  for ( IJetMaker::Jets::iterator ijet = Seeds.begin(); Seeds.end() != ijet; ++ijet ) {
    ...
    myj++;
    if ( msgLevel( MSG::DEBUG ) ) {
      debug() << "Seeds  " << myj << "| nb of combin Seeds:  " << pJet->weight()
  ...
  for ( IJetMaker::Jets::iterator ijet = ProtoJets.begin(); ProtoJets.end() != ijet; ++ijet ) {
    ...
    myj++;
    if ( msgLevel( MSG::DEBUG ) ) {
      debug() << "BSeeds " << myj << "| nb of combin Seeds:  " << pJet->weight()
              << "  nb part in the jet:  " << pJet->daughtersVector().size() << "  Pt:  " << pJet->pt() << endmsg;
      debug() << "        | m  : " << pJet->momentum() << endmsg;
      debug() << "        | eta: " << pJet->momentum().eta() << "  / phi: " << pJet->momentum().phi() << endmsg;
    }
  ...
  if ( msgLevel( MSG::INFO ) ) { counter( "#seeds" ) += Seeds.size(); }
  ...
  for ( IJetMaker::Jets::iterator ijet = MergeProtoJets.begin(); MergeProtoJets.end() != ijet; ++ijet ) {
    ...
    myj++;
    if ( msgLevel( MSG::DEBUG ) ) {
      debug() << "CSeeds " << myj << "| nb of combin Seeds:  " << pJet->weight()
              << "  nb part in the jet:  " << pJet->daughtersVector().size() << "  Pt:  " << pJet->pt() << endmsg;
      debug() << "        | m  : " << pJet->momentum() << endmsg;
      debug() << "        | eta: " << pJet->momentum().eta() << "  / phi: " << pJet->momentum().phi() << endmsg;
    }
...
//=============================================================================
IJetMaker::Jets LoKi::VVSeedFinder::JetCone( const double& Rmax, IJetMaker::Jets& seeds,
  ...
  for ( iseeds = seeds.begin(); iseeds != seeds.end(); ++iseeds ) {
    ...
    double               phiSeed = pSeed->momentum().phi();
    double               etaSeed = pSeed->momentum().eta();
...
// FILTERPROTOJETS: MERGES JETS WHICH ARE TOO CLOSE OR THE SAME
//=============================================================================
IJetMaker::Jets LoKi::VVSeedFinder::FilterProtoJets( const double& DeltaRSeeds, IJetMaker::Jets& ProtoJets ) const {
  //----------------------------------------------------------------------
        ...
        for ( IJetMaker::Jets::iterator ip2 = ip + 1; ip2 != ProtoJets.end(); ++ip2 ) {
          ...
          if ( seed2->momentum().E() > 0. ) {
            ...
            if ( testR < 0.00005 ) {
              ...
              ProtoJets.at( k )->eraseInfo( LHCb::Particle::additionalInfo::Weight );
              ProtoJets.at( k )->addInfo( LHCb::Particle::additionalInfo::Weight, myWeight );

              seed->eraseInfo( LHCb::Particle::additionalInfo::Weight );
              seed->addInfo( LHCb::Particle::additionalInfo::Weight, myWeight );
  ...
  for ( IJetMaker::Jets::iterator ip = ProtoJets.begin(); ip != ProtoJets.end(); ++ip ) {
    ...
    LHCb::Particle* seed = ( *ip );
    if ( seed->momentum().E() > 0. ) { NewProtoJets.push_back( seed ); }
  }
    ...
    while ( testJets ) {
      ...
      if ( testDR < DeltaRSeeds ) {
        if ( msgLevel( MSG::VERBOSE ) ) {
          ...
          verbose() << "Nseed phi/eta= " << ( ProtoJets.at( J1test )->momentum() ).phi() << " "
                    << ( ProtoJets.at( J1test )->momentum() ).eta() << endmsg;
          verbose() << "Nseed2phi/eta= " << ( ProtoJets.at( J2test )->momentum() ).phi() << " "
                    << ( ProtoJets.at( J2test )->momentum() ).eta() << endmsg;
          verbose() << "E P1= " << ( ProtoJets.at( J1test )->momentum() ).E()
                    << "   E P2= " << ( ProtoJets.at( J2test )->momentum() ).E() << endmsg;
          verbose() << "E P1/P2= "
                    << fabs( ( ProtoJets.at( J1test )->momentum() ).E() - ( ProtoJets.at( J2test )->momentum() ).E() ) /
                           ( ProtoJets.at( J1test )->momentum() ).E()
                    << endmsg;
        }
        if ( testDR > LHCb::Math::looseTolerance || !m_preFilter ) {

          NewProtoJets.at( J1test )->setMomentum(
              Gaudi::LorentzVector( ( ProtoJets.at( J1test )->momentum() + ProtoJets.at( J2test )->momentum() ) ) );

          const LHCb::Particle::ConstVector DaughtersJ2 = ProtoJets.at( J2test )->daughtersVector();
          for ( LHCb::Particle::ConstVector::const_iterator ip3 = DaughtersJ2.begin(); ip3 != DaughtersJ2.end();
                ++ip3 ) {
          ...
          NewProtoJets.at( J1test )->eraseInfo( LHCb::Particle::additionalInfo::Weight );
          NewProtoJets.at( J1test )->addInfo( LHCb::Particle::additionalInfo::Weight, myWeight );
          NewProtoJets.at( J1test )->eraseInfo( LHCb::Particle::additionalInfo::ConfLevel );
          NewProtoJets.at( J1test )->addInfo( LHCb::Particle::additionalInfo::ConfLevel, myCL );
...
void LoKi::VVSeedFinder::RemoveTracks( LHCb::Particle::ConstVector& particles, const LHCb::RecVertex PV ) const {
