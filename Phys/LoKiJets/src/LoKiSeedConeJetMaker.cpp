...
StatusCode LoKi::SeedConeJetMaker::makeJets( const IJetMaker::Input& input_, IJetMaker::Jets& jets_ ) const {
...
StatusCode LoKi::SeedConeJetMaker::makeJets( const IJetMaker::Input& input_, const LHCb::RecVertex& vtx_,
                                             IJetMaker::Jets& jets_ ) const {

  IJetMaker::Jets Seeds;

  // create seeds with the input particles
  m_seedFinder->makeJets( input_, vtx_, Seeds ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );

  if ( Seeds.empty() ) {
    Warning( "No Seeds from: " + m_seedFinderName ).ignore( /* AUTOMATICALLY ADDED FOR gaudi/Gaudi!763 */ );
    return StatusCode::SUCCESS;
  }
  ...
  for ( LHCb::Particle::Vector::iterator ijet = output.begin(); output.end() != ijet; ++ijet ) {
    ...
    if ( msgLevel( MSG::DEBUG ) ) {
      debug() << "ConeJets " << myj << "| nb of combin Seeds:  " << pJet->weight()
  ...
  for ( LHCb::Particle::Vector::iterator ijet = outputP.begin(); outputP.end() != ijet; ++ijet ) {
    ...
    myj++;
    if ( msgLevel( MSG::DEBUG ) ) {
      debug() << "ConeJetsP" << myj << "| nb of combin Seeds:  " << pJet->weight()
              << "  nb part in the jet:  " << pJet->daughtersVector().size() << "  Pt:  " << pJet->pt() << endmsg;
      debug() << "         | m  : " << pJet->momentum() << endmsg;
      debug() << "         | eta: " << pJet->momentum().eta() << "  / phi: " << pJet->momentum().phi() << endmsg;
    }
  ...
  if ( msgLevel( MSG::INFO ) ) { counter( "#jets" ) += outputP.size(); }
...
//=============================================================================
IJetMaker::Jets LoKi::SeedConeJetMaker::JetCone( const double& Rmax, IJetMaker::Jets& seeds,
  ...
  for ( iseeds = seeds.begin(); iseeds != seeds.end(); ++iseeds ) {
    ...
    if ( pSeed->endVertex() != NULL ) {
      Gaudi::XYZVector seedd = Gaudi::XYZVector( Gaudi::XYZPoint( pSeed->endVertex()->position() ) -
...
// JETCONE: THE HEART OF EVERYTHING, BUILDS A JET AROUND A SEED
//=============================================================================
IJetMaker::Jets LoKi::SeedConeJetMaker::JetConePurged( const double& Rmax, IJetMaker::Jets& seeds,
                                                       const IJetMaker::Input& inputs ) const {
...
// ============================================================================
/// The factory
DECLARE_COMPONENT( LoKi::SeedConeJetMaker )
// ============================================================================
// The END
// ============================================================================
